...
func VerifyBalance(isXPSXLendingFork bool, statedb *state.StateDB, lendingStateDb *LendingStateDB,
	orderType, side, status string, userAddress, relayer, lendingToken, collateralToken common.Address,
	quantity, lendingTokenDecimal, collateralTokenDecimal, lendTokenXPSPrice, collateralPrice *big.Int,
	...
	case Market, Limit:
		...
		case Investing:
			...
			case LendingStatusNew:
				...
				if lendTokenXPSPrice != nil && lendTokenXPSPrice.Sign() > 0 {
					...
					defaultFeeInXPS := common.Big0
					if lendingToken.String() != common.XPSNativeAddress {
						defaultFeeInXPS = new(big.Int).Mul(defaultFee, lendTokenXPSPrice)
						defaultFeeInXPS = new(big.Int).Div(defaultFeeInXPS, lendingTokenDecimal)
					} else {
						defaultFeeInXPS = defaultFee
					}
					if defaultFeeInXPS.Cmp(common.RelayerLendingFee) <= 0 {
		...
		case Borrowing:
			...
			case LendingStatusNew:
				...
				settleBalanceResult, err := GetSettleBalance(isXPSXLendingFork, Borrowing, lendTokenXPSPrice, collateralPrice, depositRate, borrowingFeeRate, lendingToken, collateralToken, lendingTokenDecimal, collateralTokenDecimal, quantity)
				if err != nil {
					return err
				}
