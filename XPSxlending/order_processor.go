...
func (l *Lending) processOrderList(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, lendingStateDB *lendingstate.LendingStateDB, tradingStateDb *tradingstate.TradingStateDB, side string, lendingOrderBook common.Hash, Interest *big.Int, quantityStillToTrade *big.Int, order *lendingstate.LendingItem) (*big.Int, []*lendingstate.LendingTrade, []*lendingstate.LendingItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		lendTokenXPSPrice, collateralPrice, err := l.GetCollateralPrices(header, chain, statedb, tradingStateDb, collateralToken, order.LendingToken)
		if err != nil {
			return nil, nil, nil, err
		}
		if lendTokenXPSPrice == nil || lendTokenXPSPrice.Sign() <= 0 {
		...
		tradedQuantity, collateralLockedAmount, rejectMaker, settleBalanceResult, err := l.getLendQuantity(lendTokenXPSPrice, collateralPrice, depositRate, borrowFee, coinbase, chain, header, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (l *Lending) getLendQuantity(
	...
	LendingTokenDecimal, err := l.XPSx.GetTokenDecimal(chain, statedb, makerOrder.LendingToken)
	...
	collateralTokenDecimal, err := l.XPSx.GetTokenDecimal(chain, statedb, collateralToken)
	...
	if quantity.Sign() > 0 {
		// Apply Match Order
		isXPSXLendingFork := chain.Config().IsTIPXPSXLending(header.Number)
		settleBalanceResult, err := lendingstate.GetSettleBalance(isXPSXLendingFork, takerOrder.Side, lendTokenXPSPrice, collateralPrice, depositRate, borrowFee, lendToken, collateralToken, LendingTokenDecimal, collateralTokenDecimal, quantity)
...
func (l *Lending) ProcessCancelOrder(header *types.Header, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, chain consensus.ChainContext, coinbase common.Address, lendingOrderBook common.Hash, order *lendingstate.LendingItem) (error, bool) {
	...
	lendTokenDecimal, err := l.XPSx.GetTokenDecimal(chain, statedb, originOrder.LendingToken)
	...
	if originOrder.Side == lendingstate.Borrowing {
		...
		collateralTokenDecimal, err = l.XPSx.GetTokenDecimal(chain, statedb, originOrder.CollateralToken)
	...
	if !chain.Config().IsTIPXPSXCancellationFee(header.Number) {
		...
		tokenCancelFee, tokenPriceInXPS = l.getCancelFee(chain, statedb, tradingStateDb, &originOrder, feeRate)
	}
...
func (l *Lending) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *lendingstate.LendingItem, feeRate *big.Int) (*big.Int, *big.Int) {
	...
	cancelFee, tokenPriceInXPS := common.Big0, common.Big0
	var err error
	if order.Side == lendingstate.Investing {
		cancelFee, tokenPriceInXPS, err = l.XPSx.ConvertXPSToToken(chain, statedb, tradingStateDb, order.LendingToken, common.RelayerLendingCancelFee)
	} else {
		cancelFee, tokenPriceInXPS, err = l.XPSx.ConvertXPSToToken(chain, statedb, tradingStateDb, order.CollateralToken, common.RelayerLendingCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
...
func (l *Lending) GetMediumTradePriceBeforeEpoch(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, baseToken common.Address, quoteToken common.Address) (*big.Int, error) {
	...
	if price != nil && price.Sign() > 0 {
		...
		if inversePrice != nil && inversePrice.Sign() > 0 {
			quoteTokenDecimal, err := l.XPSx.GetTokenDecimal(chain, statedb, quoteToken)
			if err != nil || quoteTokenDecimal.Sign() == 0 {
				return nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", quoteToken.String(), err)
			}
			baseTokenDecimal, err := l.XPSx.GetTokenDecimal(chain, statedb, baseToken)
...
//- Have direct pair in XPSX: lendToken/CollateralToken or CollateralToken/LendToken
//- Have pairs with XPS:
//-  lendToken/XPS and CollateralToken/XPS
//-  XPS/lendToken and XPS/CollateralToken
func (l *Lending) GetCollateralPrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, collateralToken common.Address, lendingToken common.Address) (*big.Int, *big.Int, error) {
	// lendTokenXPSPrice: price of ticker lendToken/XPS
	// collateralXPSPrice: price of ticker collateralToken/XPS
	...
	collateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().XPoS.Epoch == header.Number.Uint64()/chain.Config().XPoS.Epoch

	lendTokenXPSPrice, err := l.GetXPSBasePrices(header, chain, statedb, tradingStateDb, lendingToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralPriceUpdatedFromContract {
		...
		return lendTokenXPSPrice, collateralPriceFromContract, nil
	}
	lendingTokenDecimal, err := l.XPSx.GetTokenDecimal(chain, statedb, lendingToken)
	...
	inverseCollateralPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().XPoS.Epoch == header.Number.Uint64()/chain.Config().XPoS.Epoch
	if inverseCollateralPriceUpdatedFromContract {
		...
		return lendTokenXPSPrice, collateralPrice, nil
	}
	// if contract doesn't provide any price information
	// getting price from pair in XPSx
	...
	if lastAveragePrice != nil && lastAveragePrice.Sign() > 0 {
		log.Debug("Getting collateral/lending from direct pair in XPSx", "lendToken", lendingToken.Hex(), "collateralToken", collateralToken.Hex(), "price", lastAveragePrice)
		return lendTokenXPSPrice, lastAveragePrice, nil
	}
	collateralXPSPrice, err := l.GetXPSBasePrices(header, chain, statedb, tradingStateDb, collateralToken)
	if err != nil {
		return nil, nil, err
	}
	if collateralXPSPrice == nil || lendTokenXPSPrice == nil {
		return common.Big0, common.Big0, nil
	}
	// Calculate collateral/LendToken price from collateral/XPS, lendToken/XPS
	collateralPrice = new(big.Int).Mul(collateralXPSPrice, lendingTokenDecimal)
	collateralPrice = new(big.Int).Div(collateralPrice, lendTokenXPSPrice)
	log.Debug("GetCollateralPrices: Calculate collateral/LendToken price from collateral/XPS, lendToken/XPS", "collateralPrice", collateralPrice,
		"collateralXPSPrice", collateralXPSPrice, "lendingTokenDecimal", lendingTokenDecimal, "lendTokenXPSPrice", lendTokenXPSPrice)
	return lendTokenXPSPrice, collateralPrice, nil
}

func (l *Lending) GetXPSBasePrices(header *types.Header, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, token common.Address) (*big.Int, error) {

	tokenXPSPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, token, common.HexToAddress(common.XPSNativeAddress))
	tokenXPSPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().XPoS.Epoch == header.Number.Uint64()/chain.Config().XPoS.Epoch

	if token == common.HexToAddress(common.XPSNativeAddress) {
		return common.BasePrice, nil
	} else if tokenXPSPriceUpdatedFromContract {
		// getting lendToken price from contract first
		// otherwise, getting from XPSx lendToken/XPS
		log.Debug("Getting token/XPS price from contract", "price", tokenXPSPriceFromContract)
		return tokenXPSPriceFromContract, nil
	} else {
		XPSTokenPriceFromContract, updatedBlock := lendingstate.GetCollateralPrice(statedb, common.HexToAddress(common.XPSNativeAddress), token)
		XPSTokenPriceUpdatedFromContract := updatedBlock.Uint64()/chain.Config().XPoS.Epoch == header.Number.Uint64()/chain.Config().XPoS.Epoch
		if XPSTokenPriceUpdatedFromContract && XPSTokenPriceFromContract != nil && XPSTokenPriceFromContract.Sign() > 0 {
			// getting lendToken price from contract first
			// otherwise, getting from XPSx lendToken/XPS
			log.Debug("Getting XPS/token from contract", "price", XPSTokenPriceFromContract)
			tokenDecimal, err := l.XPSx.GetTokenDecimal(chain, statedb, token)
			log.Debug("GetTokenDecimal", "token", token.Hex(), "err", err)
			if err != nil || tokenDecimal == nil || tokenDecimal.Sign() == 0 {
				return nil, err
			}
			tokenXPSPrice := new(big.Int).Mul(common.BasePrice, tokenDecimal)
			tokenXPSPrice = new(big.Int).Div(tokenXPSPrice, XPSTokenPriceFromContract)
			return tokenXPSPrice, nil
		}
		tokenXPSPrice, err := l.GetMediumTradePriceBeforeEpoch(chain, statedb, tradingStateDb, token, common.HexToAddress(common.XPSNativeAddress))
		if err != nil {
			return nil, err
		}
		if tokenXPSPrice != nil && tokenXPSPrice.Sign() > 0 {
			log.Debug("Getting token/XPS from XPSx", "price", tokenXPSPrice, "err", err)
			return tokenXPSPrice, nil
		}
	}
	log.Debug("Can't getting tokenXPSPrice ", "token", token.Hex())
	return nil, nil
}
...
func (l *Lending) ProcessRepayLendingTrade(header *types.Header, chain consensus.ChainContext, lendingStateDB *lendingstate.LendingStateDB, statedb *state.StateDB, tradingstateDB *tradingstate.TradingStateDB, lendingBook common.Hash, lendingTradeId uint64) (trade *lendingstate.LendingTrade, err error) {
	...
	if tokenBalance.Cmp(paymentBalance) < 0 {
		...
		var err error
		if chain.Config().IsTIPXPSXLending(header.Number) {
