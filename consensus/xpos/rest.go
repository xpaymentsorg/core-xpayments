...
type GenisysClient struct {
	urlString string
	client    http.Client
	closeCh   chan struct{}
}

func NewGenisysClient(urlString string) (*GenisysClient, error) {
	h := &GenisysClient{
		urlString: urlString,
		client: http.Client{
...
func (h *GenisysClient) FetchStateSyncEvents(fromID uint64, to int64) ([]*EventRecordWithTime, error) {
...
// Fetch fetches response from genisys
func (h *GenisysClient) Fetch(rawPath string, rawQuery string) (*ResponseWithHeight, error) {
...
// FetchWithRetry returns data from genisys with retry
func (h *GenisysClient) FetchWithRetry(rawPath string, rawQuery string) (*ResponseWithHeight, error) {
	u, err := url.Parse(h.urlString)
	if err != nil {
		return nil, err
	}
	...
	for {
		...
		case <-ticker.C:
			...
			log.Info("Retrying again in 5 seconds to fetch data from Genisys", "path", u.Path)
		}
	}
}

// internal fetch method
func (h *GenisysClient) internalFetch(u *url.URL) (*ResponseWithHeight, error) {
	...
	if res.StatusCode != 200 && res.StatusCode != 204 {
		return nil, fmt.Errorf("Error while fetching data from Genisys")
	}
