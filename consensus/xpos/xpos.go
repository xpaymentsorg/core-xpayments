...
func ecrecover(header *types.Header, sigcache *lru.ARCCache, c *params.XPoSConfig) (common.Address, error) {
...
func SealHash(header *types.Header, c *params.XPoSConfig) (hash common.Hash) {
...
func encodeSigHeader(w io.Writer, header *types.Header, c *params.XPoSConfig) {
	enc := []interface{}{
		header.ParentHash,
		header.UncleHash,
		header.Coinbase,
...
func CalcProducerDelay(number uint64, succession int, c *params.XPoSConfig) uint64 {
...
// XPoSRLP returns the rlp bytes which needs to be signed for the xpos
...
func XPoSRLP(header *types.Header, c *params.XPoSConfig) []byte {
	b := new(bytes.Buffer)
	encodeSigHeader(b, header, c)
	return b.Bytes()
}

// XPoS is the xpayments-xpos consensus engine
type XPoS struct {
	chainConfig *params.ChainConfig // Chain config
	config      *params.XPoSConfig  // Consensus engine configuration parameters for xpos consensus
...
// New creates a xPayments XPoS consensus engine.
func New(
	...
	// get xpos config
	xposConfig := chainConfig.XPoS

	// Set any missing consensus parameters to their defaults
	if xposConfig != nil && xposConfig.Sprint == 0 {
		xposConfig.Sprint = defaultSprintLength
	}
	...
	genisysClient, _ := NewGenisysClient(genisysURL)
	genesisContractsClient := NewGenesisContractsClient(chainConfig, xposConfig.ValidatorContract, xposConfig.StateReceiverContract, ethAPI)
	c := &XPoS{
		...
		GenesisContractsClient: genesisContractsClient,
		GenisysClient:          genisysClient,
		WithoutGenisys:         withoutGenisys,
	}

	// make sure we can decode all the GenesisAlloc in the XPoSConfig.
...
func (c *XPoS) Author(header *types.Header) (common.Address, error) {
	return ecrecover(header, c.signatures, c.config)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *XPoS) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
	return c.verifyHeader(chain, header, nil)
}

// VerifyHeaders is similar to VerifyHeader, but verifies a batch of headers. The
// method returns a quit channel to axpost the operations and a results channel to
// retrieve the async verifications (the order is that of the input slice).
func (c *XPoS) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	axpost := make(chan struct{})
...
func (c *XPoS) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *XPoS) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *XPoS) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *XPoS) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *XPoS) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *XPoS) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *XPoS) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// rewards given.
func (c *XPoS) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) {
	stateSyncData := []*types.StateSyncData{}

	var err error
	headerNumber := header.Number.Uint64()
	if headerNumber%c.config.Sprint == 0 {
		cx := chainContext{Chain: chain, XPoS: c}
...
func (c *XPoS) changeContractCodeIfNeeded(headerNumber uint64, state *state.StateDB) error {
...
func (c *XPoS) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	stateSyncData := []*types.StateSyncData{}
...
// with.
func (c *XPoS) Authorize(signer common.Address, signFn SignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *XPoS) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: signer}, accounts.MimetypeXPoS, XPoSRLP(header, c.config))
	if err != nil {
		return err
	}
...
// current signer.
func (c *XPoS) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
// SealHash returns the hash of a block prior to it being sealed.
func (c *XPoS) SealHash(header *types.Header) common.Hash {
	return SealHash(header, c.config)
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the signer voting.
func (c *XPoS) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "xpos",
		Version:   "1.0",
		Service:   &API{chain: chain, xpos: c},
		Public:    false,
	}}
}

// Close implements consensus.Engine. It's a noop for xpos as there are no background threads.
func (c *XPoS) Close() error {
	c.GenisysClient.Close()
	return nil
}

// GetCurrentSpan get current span from contract
func (c *XPoS) GetCurrentSpan(headerHash common.Hash) (*Span, error) {
	// block
...
func (c *XPoS) GetCurrentValidators(headerHash common.Hash, blockNumber uint64) ([]*Validator, error) {
	// block
	blockNr := rpc.BlockNumberOrHashWithHash(headerHash, false)

	// method
	method := "getXPoSValidators"
...
func (c *XPoS) checkAndCommitSpan(
	state *state.StateDB,
	header *types.Header,
...
func (c *XPoS) needToCommitSpan(span *Span, headerNumber uint64) bool {
	// if span is nil
	if span == nil {
		return false
	}
...
func (c *XPoS) fetchAndCommitSpan(
	newSpanID uint64,
	state *state.StateDB,
	header *types.Header,
	chain core.ChainContext,
) error {
	var genisysSpan GenisysSpan

	if c.WithoutGenisys {
		s, err := c.getNextGenisysSpanForTest(newSpanID, state, header, chain)
		if err != nil {
			return err
		}
		genisysSpan = *s
	} else {
		response, err := c.GenisysClient.FetchWithRetry(fmt.Sprintf("xpos/span/%d", newSpanID), "")
		if err != nil {
			return err
		}

		if err := json.Unmarshal(response.Result, &genisysSpan); err != nil {
			return err
		}
	}

	// check if chain id matches with genisys span
	if genisysSpan.ChainID != c.chainConfig.ChainID.String() {
		return fmt.Errorf(
			"Chain id proposed span, %s, and xpos chain id, %s, doesn't match",
			genisysSpan.ChainID,
	...
	for _, val := range genisysSpan.ValidatorSet.Validators {
	...
	for _, val := range genisysSpan.SelectedProducers {
	...
	log.Info("✅ Committing new span",
		"id", genisysSpan.ID,
		"startBlock", genisysSpan.StartBlock,
		"endBlock", genisysSpan.EndBlock,
	...
	// get packed data
	data, err := c.validatorSetABI.Pack(method,
		big.NewInt(0).SetUint64(genisysSpan.ID),
		big.NewInt(0).SetUint64(genisysSpan.StartBlock),
		big.NewInt(0).SetUint64(genisysSpan.EndBlock),
		validatorBytes,
		producerBytes,
	)
	if err != nil {
...
func (c *XPoS) CommitStates(
	...
	log.Info(
		"Fetching state updates from Genisys",
		"fromID", lastStateID+1,
		"to", to.Format(time.RFC3339))
	eventRecords, err := c.GenisysClient.FetchStateSyncEvents(lastStateID+1, to.Unix())
...
func (c *XPoS) SetGenisysClient(h IGenisysClient) {
	c.GenisysClient = h
}
...
func (c *XPoS) getNextGenisysSpanForTest(
	newSpanID uint64,
	state *state.StateDB,
	header *types.Header,
	chain core.ChainContext,
