...
import (
	...
	"github.com/xpaymentsorg/go-xpayments/internal/testlog"
...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom xpsash engine.
	xpsash := NewTester([]string{server.URL}, false)
	defer xpsash.Close()
	...
	xpsash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := xpsash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	xpsash := New(config, []string{server.URL}, false)
	defer xpsash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom xpsash engine.
	xpsash := NewTester([]string{server.URL}, false)
	xpsash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer xpsash.Close()
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			xpsash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, xpsash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch xpsash result timeout", id+1)
		}
	}
}
