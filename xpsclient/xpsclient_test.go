...
// Verify that Client implements the xpayments interfaces.
var (
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create xPayments Service
	config := &xpsconfig.Config{Genesis: genesis}
	config.Xpsash.PowMode = xpsash.ModeFake
	xpsservice, err := xps.New(n, config)
	if err != nil {
		t.Fatalf("can't create new xpayments service: %v", err)
	}
	// Import the test chain.
	if err := n.Start(); err != nil {
		t.Fatalf("can't start test node: %v", err)
	}
	if _, err := xpsservice.BlockChain().InsertChain(blocks[1:]); err != nil {
...
func TestXpsClient(t *testing.T) {
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == xpayments.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != xpayments.NotFound {
		t.Fatal("error should be xpayments.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllXpsashProtocolChanges.ChainID) != 0 {
