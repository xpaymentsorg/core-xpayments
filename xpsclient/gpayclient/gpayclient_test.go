...
func TestGpayClient(t *testing.T) {
...
func testGetProof(t *testing.T, client *rpc.Client) {
	ec := New(client)
	xpscl := xpsclient.NewClient(client)
	...
	// test nonce
	nonce, _ := xpscl.NonceAt(context.Background(), result.Address, nil)
	if result.Nonce != nonce {
		t.Fatalf("invalid nonce, want: %v got: %v", nonce, result.Nonce)
	}
	// test balance
	balance, _ := xpscl.BalanceAt(context.Background(), result.Address, nil)
...
func testSubscribePendingTransactions(t *testing.T, client *rpc.Client) {
	...
	// Send a transaction
	chainID, err := xpscl.ChainID(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	// Create transaction
	...
	// Send transaction
	err = xpscl.SendTransaction(context.Background(), signedTx)
	if err != nil {
		t.Fatal(err)
	}
