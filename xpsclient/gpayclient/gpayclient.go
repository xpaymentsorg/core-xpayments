...
// Package gpayclient provides an RPC client for gpay-specific APIs.
package gpayclient
...
// Client is a wrapper around rpc.Client that implements gpay-specific functionality.
//
// If you want to use the standardized xPayments RPC functionality, use xpsclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg xpayments.CallMsg) (*types.AccessList, uint64, string, error) {
	...
	if err := ec.c.CallContext(ctx, &result, "xps_createAccessList", toCallArg(msg)); err != nil {
		return nil, 0, "", err
	}
...
func (ec *Client) GetProof(ctx context.Context, account common.Address, keys []string, blockNumber *big.Int) (*AccountResult, error) {
	...
	err := ec.c.CallContext(ctx, &res, "xps_getProof", account, keys, toBlockNumArg(blockNumber))
...
// the message call.
// Please use xpsclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg xpayments.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
	var hex hexutil.Bytes
	err := ec.c.CallContext(
		ctx, &hex, "xps_call", toCallArg(msg),
		toBlockNumArg(blockNumber), toOverrideMap(overrides),
	)
	return hex, err
}

// GCStats retrieves the current garbage collection stats from a gpay node.
...
// MemStats retrieves the current memory stats from a gpay node.
...
// GetNodeInfo retrieves the node info of a gpay node.
...
func (ec *Client) SubscribePendingTransactions(ctx context.Context, ch chan<- common.Hash) (*rpc.ClientSubscription, error) {
	return ec.c.XpsSubscribe(ctx, ch, "newPendingTransactions")
}
