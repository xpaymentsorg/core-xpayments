...
import (
	...
	simulation "github.com/xpaymentsorg/go-xpayments/contracts/XPSx/testnet"
...
func main() {
	...
	trc21IssuerAddr, trc21Issuer, err := XPSx.DeployTRC21Issuer(auth, client, simulation.MinTRC21Apply)
	if err != nil {
	...
	//init XPSX Listing in
	auth.Nonce = big.NewInt(int64(nonce + 1))
	XPSxListtingAddr, XPSxListing, err := XPSx.DeployXPSXListing(auth, client)
	if err != nil {
		log.Fatal("DeployXPSXListing", err)
	}
	XPSxListing.TransactOpts.GasPrice = big.NewInt(250000000000000)

	fmt.Println("===> XPSx listing address", XPSxListtingAddr.Hex())
	fmt.Println("wait 10s to execute init smart contract : XPSx listing ")
	time.Sleep(2 * time.Second)

	// init Relayer Registration
	auth.Nonce = big.NewInt(int64(nonce + 2))
	relayerRegistrationAddr, relayerRegistration, err := XPSx.DeployRelayerRegistration(auth, client, XPSxListtingAddr, simulation.MaxRelayers, simulation.MaxTokenList, simulation.MinDeposit)
	if err != nil {
	...
	lendingRelayerRegistrationAddr, lendingRelayerRegistration, err := XPSx.DeployLendingRelayerRegistration(auth, client, relayerRegistrationAddr, XPSxListtingAddr)
	if err != nil {
	...
	applyXPSXListing(XPSxListing, tokenList, currentNonce)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddCollateral(simulation.XPSNative, simulation.CollateralDepositRate, simulation.CollateralLiquidationRate, simulation.CollateralRecallRate)
	...
	nonce = nonce + 1
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	_, err = lendingRelayerRegistration.AddBaseToken(simulation.XPSNative)
	if err != nil {
		log.Fatal("Lending add base token XPS", err)
	}
	...
	relayerRegistration, err = XPSx.NewRelayerRegistration(ownerRelayer, relayerRegistrationAddr, client)
	if err != nil {
	...
	/*
		for _, token := range tokenList {
			...
			toTokens = append(toTokens, simulation.XPSNative)
		}
	*/

	// XPS/BTC
	fromTokens = append(fromTokens, simulation.XPSNative)
	...
	fmt.Println("wait 2s to apply token to list XPSx")
	time.Sleep(2 * time.Second)

	// Lending apply
	nonce = nonce + 1
	lendingRelayerRegistration, err = XPSx.NewLendingRelayerRegistration(ownerRelayer, lendingRelayerRegistrationAddr, client)
	if err != nil {
		log.Fatal("NewRelayerRegistration", err)
	}
	lendingRelayerRegistration.TransactOpts.Nonce = big.NewInt(int64(nonce))
	...
	// XPS 1 min
	baseTokens = append(baseTokens, simulation.XPSNative)
	terms = append(terms, big.NewInt(60))
	collaterals = append(collaterals, common.HexToAddress("0x0"))
...
func initTRC21(auth *bind.TransactOpts, client *ethclient.Client, nonce uint64, tokenNameList []string) []map[string]interface{} {
	...
	for _, tokenName := range tokenNameList {
		...
		if tokenName == "ADA" {
			d = 0
			tokenCap = new(big.Int).Div(simulation.TRC21TokenCap, simulation.BaseXPS)
		}
		if tokenName == "USDT" {
			d = 6
		...
		tokenAddr, _, err := XPSx.DeployTRC21(auth, client, simulation.Owners, simulation.Required, tokenName, tokenName, d, tokenCap, simulation.TRC21TokenFee, depositFee, withdrawFee)
		if err != nil {
...
func applyIssuer(trc21Issuer *XPSx.TRC21Issuer, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
...
func applyXPSXListing(XPSxListing *XPSx.XPSXListing, tokenList []map[string]interface{}, nonce uint64) {
	for _, token := range tokenList {
		XPSxListing.TransactOpts.Nonce = big.NewInt(int64(nonce))
		XPSxListing.TransactOpts.Value = simulation.XPSXListingFee
		_, err := XPSxListing.Apply(token["address"].(common.Address))
		if err != nil {
			log.Fatal("XPSxListing Apply ", token["name"].(string), err)
		}
		fmt.Println("applyXPSXListing ", token["name"].(string))
		nonce = nonce + 1
	}
	time.Sleep(5 * time.Second)
}
...
func airdrop(auth *bind.TransactOpts, client *ethclient.Client, tokenList []map[string]interface{}, addresses []common.Address, nonce uint64) {
	for _, token := range tokenList {
		for _, address := range addresses {
			trc21Contract, _ := XPSx.NewTRC21(auth, token["address"].(common.Address), client)
