...
contract RelayerRegistration {
    ...
    address constant private XPSNative = 0x0000000000000000000000000000000000000001;
    ...
    AbstractXPSXListing private XPSXListing;
    ...
    constructor (address XPSxListing, uint maxRelayers, uint maxTokenList, uint minDeposit) public {
        XPSXListing = AbstractXPSXListing(XPSxListing);
        RelayerCount = 0;
    ...
    function register(address coinbase, uint16 tradeFee, address[] memory fromTokens, address[] memory toTokens) public payable {
        ...
        // check valid tokens, token must pair with XPS(x/XPS)
    ...
    function depositMore(address coinbase) public payable relayerOwnerOnly(coinbase) onlyActiveRelayer(coinbase) notForSale(coinbase) nonZeroValue {
        require(msg.value >= 1 ether, "At least 1 XPS is required for a deposit request");
    ...
    function indexOf(address[] memory XPSPair, address target) internal pure returns (bool){
        for (uint i = 0; i < XPSPair.length; i ++) {
            if (XPSPair[i] == target) {
                return true;
            }
        }
    ...
    function validateTokens(address[] memory fromTokens, address[] memory toTokens) internal returns(bool) {
        ...
        address[] memory XPSPairs = new address[](fromTokens.length);
        address[] memory nonXPSPairs = new address[](fromTokens.length);

        for (uint i = 0; i < toTokens.length; i++) {
            bool b = XPSXListing.getTokenStatus(toTokens[i]) || (toTokens[i] == XPSNative);
            b = b && (XPSXListing.getTokenStatus(fromTokens[i]) || fromTokens[i] == XPSNative);
            if (!b) {
                return false;
            }
            if (toTokens[i] == XPSNative) {
                XPSPairs[countPair] = fromTokens[i];
                countPair++;
            } else {
                if (fromTokens[i] == XPSNative) {
                    XPSPairs[countPair] = toTokens[i];
                    countPair++;
                }
                nonXPSPairs[countNonPair] = toTokens[i];
                countNonPair++;
            }
        }

        for (uint j = 0; j < countNonPair; j++) {
            if (!indexOf(XPSPairs, nonXPSPairs[j])) {
                return false;
            }
        }
        return true;
    ...
    function addingTokenValidation(
        ...
        address[] memory XPSPairs = new address[](RELAYER_LIST[coinbase]._toTokens.length + 1);

        bool b = XPSXListing.getTokenStatus(toToken) || (toToken == XPSNative);
        b = b && (XPSXListing.getTokenStatus(fromToken) || fromToken == XPSNative);
        if (!b) {
            return false;
        }

        if (fromToken == XPSNative || toToken == XPSNative) {
            return true;
        }

        // get tokens that paired with XPS
        for (uint i = 0; i < RELAYER_LIST[coinbase]._toTokens.length; i++) {
            if (RELAYER_LIST[coinbase]._toTokens[i] == XPSNative) {
                XPSPairs[countPair] = RELAYER_LIST[coinbase]._fromTokens[i];
                countPair++;
            } else {
                if (RELAYER_LIST[coinbase]._fromTokens[i] == XPSNative) {
                    XPSPairs[countPair] = RELAYER_LIST[coinbase]._toTokens[i];
                    countPair++;
                }
            }
        }
        if (!indexOf(XPSPairs, toToken)) {
            return false;
        }
        return true;
    }
