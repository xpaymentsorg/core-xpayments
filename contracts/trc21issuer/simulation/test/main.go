...
import (
	...
	"github.com/xpaymentsorg/go-xpayments/contracts/trc21issuer"
	"github.com/xpaymentsorg/go-xpayments/contracts/trc21issuer/simulation"
...
func airDropTokenToAccountNoXPS() {
...
func testTransferTRC21TokenWithAccountNoXPS() {
	client, err := ethclient.Dial(simulation.RpcEndpoint)
	if err != nil {
		fmt.Println(err, client)
	}

	// access to address which received token trc20 but dont have XPS
...
func main() {
	...
	for i := 0; i < 10000000; i++ {
		airDropTokenToAccountNoXPS()
		fmt.Println("Finish airdrop token to a account")
		testTransferTRC21TokenWithAccountNoXPS()
		fmt.Println("Finish transfer trc21 token with a account no XPS")
		testTransferTrc21Fail()
		fmt.Println("Finish testing ! Success transferAmount token trc20 with a account no XPS")
	}
