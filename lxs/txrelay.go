...
func newLxsTxRelay(ps *serverPeerSet, retriever *retrieveManager) *lxsTxRelay {
	r := &lxsTxRelay{
...
func (ltrx *lxsTxRelay) Stop() {
	close(ltrx.stop)
}

func (ltrx *lxsTxRelay) registerPeer(p *serverPeer) {
	ltrx.lock.Lock()
...
func (ltrx *lxsTxRelay) unregisterPeer(p *serverPeer) {
	ltrx.lock.Lock()
	defer ltrx.lock.Unlock()
...
func (ltrx *lxsTxRelay) send(txs types.Transactions, count int) {
...
func (ltrx *lxsTxRelay) Send(txs types.Transactions) {
	ltrx.lock.Lock()
	defer ltrx.lock.Unlock()
...
func (ltrx *lxsTxRelay) NewHead(head common.Hash, mined []common.Hash, rollback []common.Hash) {
	ltrx.lock.Lock()
	defer ltrx.lock.Unlock()
...
func (ltrx *lxsTxRelay) Discard(hashes []common.Hash) {
	ltrx.lock.Lock()
	defer ltrx.lock.Unlock()
