...
type LxsOdrRequest interface {
	GetCost(*serverPeer) uint64
	CanSend(*serverPeer) bool
	Request(uint64, *serverPeer) error
	Validate(xpsdb.Database, *Msg) error
}

func LxsRequest(req light.OdrRequest) LxsOdrRequest {
	switch r := req.(type) {
...
// peer's cost table (implementation of LxsOdrRequest)
...
// Request sends an ODR request to the LXS network (implementation of LxsOdrRequest)
...
// Valid processes an ODR request reply message from the LXS network
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LxsOdrRequest)
func (r *BlockRequest) Validate(db xpsdb.Database, msg *Msg) error {
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LxsOdrRequest)
func (r *ReceiptsRequest) Validate(db xpsdb.Database, msg *Msg) error {
...
// ODR request type for state/storage trie entries, see LxsOdrRequest interface
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LxsOdrRequest)
func (r *TrieRequest) Validate(db xpsdb.Database, msg *Msg) error {
...
// ODR request type for node data (used for retrieving contract code), see LxsOdrRequest interface
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LxsOdrRequest)
func (r *CodeRequest) Validate(db xpsdb.Database, msg *Msg) error {
...
const (
	...
	// htAuxNone = 1 ; deprecated number, used in lxs2/3 previously.
...
// ODR request type for requesting headers by Canonical Hash Trie, see LxsOdrRequest interface
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LxsOdrRequest)
func (r *ChtRequest) Validate(db xpsdb.Database, msg *Msg) error {
...
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LxsOdrRequest)
func (r *BloomRequest) Validate(db xpsdb.Database, msg *Msg) error {
...
// Validate processes an ODR request reply message from the LXS network
// returns true and stores results in memory if the message was a valid reply
// to the request (implementation of LxsOdrRequest)
func (r *TxStatusRequest) Validate(db xpsdb.Database, msg *Msg) error {
