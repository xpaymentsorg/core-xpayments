...
// serverBackend defines the backend functions needed for serving LXS requests
...
// RequestType is a static struct that describes an LXS request type and references
...
// Lxs3 contains the request types supported by lxs/2 and lxs/3
var Lxs3 = map[uint64]RequestType{
	GetBlockHeadersMsg: {
...
func handleGetHelperTrieProofs(msg Decoder) (serveRequestFn, uint64, uint64, error) {
	...
	return func(backend serverBackend, p *clientPeer, waitOrStop func() bool) *reply {
		...
		for i, request := range r.Reqs {
			...
			// legacy lxs protocol and drop this hack when the lxs2/3 are
			// not supported.
