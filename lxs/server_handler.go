...
func newServerHandler(server *LxsServer, blockchain *core.BlockChain, chainDb xpsdb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	// that "non-synced" lxs-server peers are still allowed to keep the connection.
	if !h.synced() {
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light xpayments response", "err", err)
			return err
		default:
		}
