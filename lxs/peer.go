...
import (
	...
	"github.com/xpaymentsorg/go-xpayments/lxs/flowcontrol"
	"github.com/xpaymentsorg/go-xpayments/lxs/utils"
	vfc "github.com/xpaymentsorg/go-xpayments/lxs/vflux/client"
	vfs "github.com/xpaymentsorg/go-xpayments/lxs/vflux/server"
...
const (
	...
	// handshakeTimeout is the timeout LXS handshake will be treated as failed.
...
func (p *peerCommons) String() string {
	return fmt.Sprintf("Peer %s [%s]", p.id, fmt.Sprintf("lxs/%d", p.version))
}

// PeerInfo represents a short summary of the `xps` sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // xPayments protocol version negotiated
...
// handshake executes the lxs protocol handshake, negotiating version number,
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// If the protocol version is beyond lxs4, then pass the forkID
	// as well. Check http://eips.xpayments.org/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
	...
	// Check forkID if the protocol version is beyond the lxs4
	if p.version >= lpv4 {
...
// The node here refers to the lxs server.
	...
	return p.handshake(common.Big0, common.Hash{}, 0, genesis, forkid, forkFilter, func(lists *keyValueList) {
		...
		*lists = (*lists).add("announceType", p.announceType)
		...
		if p.version >= lpv4 {
			...
			// The weak assumption is held here that legacy lxs server(lxs2,3)
			// has unlimited transaction history. The lxs serving in these legacy
...
// clientPeer represents each node to which the lxs server is connected.
...
func (p *clientPeer) InactiveAllowance() time.Duration {
	return 0 // will return more than zero for lxs/5 clients
}
...
func (p *clientPeer) Handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, server *LxsServer) error {
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local xpayments node is running in archive mode, advertise ourselves we have
...
// participating in the Light xPayments sub-protocol.
...
// serverSet is a special set which contains all connected lxs servers.
// Lxs servers will also be discovered by discovery protocol because they
// also run the LXS protocol. We can't drop them although they are useless
// for us(server) but for other protocols(e.g. XPS) upon the devp2p they
// may be useful.
