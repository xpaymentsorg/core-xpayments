...
func TestSetHeadBeforeTotalDifficulty(t *testing.T) {
	...
	n, lxsService := startLxsService(t, genesis, headers)
	defer n.Close()

	api := NewConsensusAPI(lxsService)
...
func TestExecutePayloadV1(t *testing.T) {
	genesis, headers, blocks := generatePreMergeChain(10)
	n, lxsService := startLxsService(t, genesis, headers[:9])
	lxsService.Merger().ReachTTD()
	defer n.Close()
...
func startLxsService(t *testing.T, genesis *core.Genesis, headers []*types.Header) (*node.Node, *lxs.LightxPayments) {
	t.Helper()
	...
	lxsService, err := lxs.New(n, xpscfg)
	if err != nil {
	...
	if _, err := lxsService.BlockChain().InsertHeaderChain(headers, 0); err != nil {
		n.Close()
