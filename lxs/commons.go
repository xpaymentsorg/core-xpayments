...
// lxsCommons contains fields needed by both server and client.
type lxsCommons struct {
	genesis                      common.Hash
	config                       *xpsconfig.Config
	chainConfig                  *params.ChainConfig
	iConfig                      *light.IndexerConfig
	chainDb, lxsDb               xpsdb.Database
...
type NodeInfo struct {
	Network    uint64                   `json:"network"`    // xPayments network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
...
// makeProtocols creates protocol descriptors for the given LXS versions.
func (c *lxsCommons) makeProtocols(versions []uint, runPeer func(version uint, p *p2p.Peer, rw p2p.MsgReadWriter) error, peerInfo func(id enode.ID) interface{}, dialCandidates enode.Iterator) []p2p.Protocol {
...
func (c *lxsCommons) nodeInfo() interface{} {
...
func (c *lxsCommons) latestLocalCheckpoint() params.TrustedCheckpoint {
...
func (c *lxsCommons) localCheckpoint(index uint64) params.TrustedCheckpoint {
...
func (c *lxsCommons) setupOracle(node *node.Node, genesis common.Hash, xpsconfig *xpsconfig.Config) *checkpointoracle.CheckpointOracle {
	config := xpsconfig.CheckpointOracle
	if config == nil {
	...
	client := xpsclient.NewClient(rpcClient)
	oracle.Start(client)
