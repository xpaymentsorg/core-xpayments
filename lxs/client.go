...
import (
	...
	"github.com/xpaymentsorg/go-xpayments/lxs/vflux"
...
func New(stack *node.Node, config *xpsconfig.Config) (*LightxPayments, error) {
	...
	lxsDb, err := stack.OpenDatabase("lxs.client", 0, 0, "xps/db/lxsclient/", false)
	if err != nil {
		return nil, err
	}
	...
	lxps.serverPool, lxps.serverPoolIterator = vfc.NewServerPool(lxsDb, []byte("serverpool:"), time.Second, prenegQuery, &mclock.System{}, config.UltraLightServers, requestList)
	...
	lxps.relay = newLxsTxRelay(peers, lxps.retriever)

	lxps.odr = NewLxsOdr(chainDb, light.DefaultClientIndexerConfig, lxps.peers, lxps.retriever)
	...
	lxps.ApiBackend = &LxsApiBackend{stack.Config().ExtRPCEnabled(), stack.Config().AllowUnprotectedTxs, lxps, nil}
...
// vfxVersion returns the version number of the "lxs" service subdomain of the vflux UDP
...
func (s *LightxPayments) vfxVersion(n *enode.Node) uint {
	...
	var lxs []rlp.RawValue
	if err := n.Load(enr.WithEntry("lxs", &lxs)); err != nil || len(lxs) < 1 {
		return 0
	}
	var version uint
	rlp.DecodeBytes(lxs[0], &version) // Ignore additional fields (for forward compatibility).
	return version
}
...
func (s *LightxPayments) prenegQuery(n *enode.Node) int {
	...
	requests.Add("lxs", vflux.CapacityQueryName, vflux.CapacityQueryReq{
		Bias:      180,
	...
	return append(apis, []rpc.API{
		{
			...
			Namespace: "lxs",
			Version:   "1.0",
			Service:   NewPrivateLightAPI(&s.lxsCommons),
			Public:    false,
		}, {
			Namespace: "vflux",
			Version:   "1.0",
...
func (s *LightxPayments) LxsVersion() int                    { return int(ClientProtocolVersions[0]) }
