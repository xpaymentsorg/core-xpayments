...
import (
	...
	xpsdownloader "github.com/xpaymentsorg/go-xpayments/xps/downloader"
...
func getHead(ctx context.Context, t *testing.T, client *rpc.Client) (uint64, common.Hash) {
	...
	if err := client.CallContext(ctx, &res, "xps_getBlockByNumber", "latest", false); err != nil {
...
func testRequest(ctx context.Context, t *testing.T, client *rpc.Client) bool {
	...
	defer cancel()
	err := client.CallContext(c, &res, "xps_getBalance", addr, "latest")
	if err != nil {
...
func setCapacity(ctx context.Context, t *testing.T, server *rpc.Client, clientID enode.ID, cap uint64) {
	...
	params["capacity"] = cap
	if err := server.CallContext(ctx, nil, "lxs_setClientParams", []enode.ID{clientID}, []string{}, params); err != nil {
...
func getCapacity(ctx context.Context, t *testing.T, server *rpc.Client, clientID enode.ID) uint64 {
	...
	if err := server.CallContext(ctx, &res, "lxs_clientInfo", []enode.ID{clientID}, []string{}); err != nil {
...
func getCapacityInfo(ctx context.Context, t *testing.T, server *rpc.Client) (minCap, totalCap uint64) {
	...
	if err := server.CallContext(ctx, &res, "lxs_serverInfo"); err != nil {
...
var services = adapters.LifecycleConstructors{
	"lxsclient": newLxsClientService,
	"lxsserver": newLxsServerService,
}
...
func NewAdapter(adapterType string, services adapters.LifecycleConstructors) (adapter adapters.NodeAdapter, teardown func(), err error) {
	...
	case "exec":
		baseDir, err0 := ioutil.TempDir("", "lxs-test")
		if err0 != nil {
...
func testSim(t *testing.T, serverCount, clientCount int, serverDir, clientDir []string, test func(ctx context.Context, net *simulations.Network, servers []*simulations.Node, clients []*simulations.Node) bool) bool {
	...
	for i := range clients {
		...
		clientconf.Lifecycles = []string{"lxsclient"}
	...
	for i := range servers {
		...
		serverconf.Lifecycles = []string{"lxsserver"}
...
func newLxsClientService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	config := xpsconfig.Defaults
	config.SyncMode = (xpsdownloader.SyncMode)(downloader.LightSync)
	config.Xpsash.PowMode = xpsash.ModeFake
	return New(stack, &config)
}

func newLxsServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	config := xpsconfig.Defaults
	config.SyncMode = (xpsdownloader.SyncMode)(downloader.FullSync)
	config.LightServ = testServerCapacity
	config.LightPeers = testMaxClients
	xpayments, err := xps.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLxsServer(stack, xpayments, &config)
	if err != nil {
		return nil, err
	}
	return xpayments, nil
}
