...
// LxsOdr implements light.OdrBackend
type LxsOdr struct {
	db                                         xpsdb.Database
...
func NewLxsOdr(db xpsdb.Database, config *light.IndexerConfig, peers *serverPeerSet, retriever *retrieveManager) *LxsOdr {
	return &LxsOdr{
		db:            db,
...
func (odr *LxsOdr) Stop() {
	close(odr.stop)
}

// Database returns the backing database
func (odr *LxsOdr) Database() xpsdb.Database {
	return odr.db
}

// SetIndexers adds the necessary chain indexers to the ODR backend
func (odr *LxsOdr) SetIndexers(chtIndexer, bloomTrieIndexer, bloomIndexer *core.ChainIndexer) {
...
func (odr *LxsOdr) ChtIndexer() *core.ChainIndexer {
	return odr.chtIndexer
}

// BloomTrieIndexer returns the bloom trie chain indexer
func (odr *LxsOdr) BloomTrieIndexer() *core.ChainIndexer {
	return odr.bloomTrieIndexer
}

// BloomIndexer returns the bloombits chain indexer
func (odr *LxsOdr) BloomIndexer() *core.ChainIndexer {
	return odr.bloomIndexer
}

// IndexerConfig returns the indexer config.
func (odr *LxsOdr) IndexerConfig() *light.IndexerConfig {
...
// Msg encodes a LXS message that delivers reply data for a request
type Msg struct {
	MsgType int
	ReqID   uint64
	Obj     interface{}
...
const (
	...
	maxTxStatusCandidates = 5 // The maximum lxs servers the tx status requests will be sent to.
)

// RetrieveTxStatus retrieves the transaction status from the LXS network.
// There is no guarantee in the LXS protocol that the mined transaction will
...
func (odr *LxsOdr) RetrieveTxStatus(ctx context.Context, req *light.TxStatusRequest) error {
...
// Retrieve tries to fetch an object from the LXS network. It's a common API
// for most of the LXS requests except for the TxStatusRequest which needs
// the additional retry mechanism.
// If the network retrieval was successful, it stores the object in local db.
func (odr *LxsOdr) Retrieve(ctx context.Context, req light.OdrRequest) (err error) {
	lreq := LxsRequest(req)
