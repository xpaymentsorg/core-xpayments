...
func NewLxsServer(node *node.Node, e xpsBackend, config *xpsconfig.Config) (*LxsServer, error) {
	lxsDb, err := node.OpenDatabase("lxs.server", 0, 0, "xps/db/lxsserver/", false)
	if err != nil {
		return nil, err
	}
	...
	// LXS flow control tries to more or less guarantee the possibility for the
	...
	srv.clientPool = vfs.NewClientPool(lxsDb, srv.minCapacity, defaultConnectedBias, mclock.System{}, issync)
	srv.clientPool.Start()
	srv.clientPool.SetDefaultFactors(defaultPosFactors, defaultNegFactors)
	srv.vfluxServer.Register(srv.clientPool, "lxs", "xPayments light client service")
...
func (s *LxsServer) Protocols() []p2p.Protocol {
	...
	// Add "lxs" ENR entries.
	for i := range ps {
		ps[i].Attributes = []enr.Entry{&lxsEntry{
			VfxVersion: 1,
		}}
	}
	return ps
...
// Start starts the LXS server
func (s *LxsServer) Start() error {
...
// Stop stops the LXS service
func (s *LxsServer) Stop() error {
	close(s.closeCh)
...
func (s *LxsServer) capacityManagement() {
	defer s.wg.Done()
