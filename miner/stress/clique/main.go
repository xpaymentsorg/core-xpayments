...
func main() {
	...
	for _, sealer := range sealers {
		...
		stack, xpsBackend, err := makeSealer(genesis)
		if err != nil {
			panic(err)
		}
		defer stack.Close()
...
func makeSealer(genesis *core.Genesis) (*node.Node, *xps.xPayments, error) {
	...
	// Start the node and configure a full xPayments node on it
