...
// PublicxPaymentsAPI provides an API to access xPayments related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicxPaymentsAPI struct {
	b Backend
}

// NewPublicxPaymentsAPI creates a new xPayments protocol API.
func NewPublicxPaymentsAPI(b Backend) *PublicxPaymentsAPI {
	return &PublicxPaymentsAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicxPaymentsAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicxPaymentsAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicxPaymentsAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *PublicxPaymentsAPI) Syncing() (interface{}, error) {
...
// Sign calculates an xPayments ECDSA signature for:
// keccack256("\x19xPayments Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/xpayments/go-xpayments/wiki/Management-APIs#personal_sign
...
// Note, this function is compatible with xps_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19xPayments Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/xpayments/go-xpayments/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid xPayments signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gpay.
...
// PublicBlockChainAPI provides an API to access the xPayments blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new xPayments blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current xpayments chain config.
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	defer func(start time.Time) { log.Debug("Executing XVM call finished", "runtime", time.Since(start)) }(time.Now())
	...
	// Get a new instance of the XVM.
	msg, err := args.ToMessage(globalGasCap, header.BaseFee)
	if err != nil {
		return nil, err
	}
	xvm, vmError, err := b.GetXVM(ctx, msg, state, header, &vm.Config{NoBaseFee: true})
	if err != nil {
		return nil, err
	}
	// Wait for the context to be done and cancel the xvm. Even if the
	// XVM has finished, cancelling may be done (repeatedly)
	go func() {
		<-ctx.Done()
		xvm.Cancel()
	}()
	...
	result, err := core.ApplyMessage(xvm, msg, gp)
...
// revertError is an API error that encompassas an XVM revertal with JSON error
...
// See: https://github.com/xpayments/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
func (s *PublicBlockChainAPI) Call(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride) (hexutil.Bytes, error) {
	result, err := DoCall(ctx, s.b, args, blockNrOrHash, overrides, s.b.RPCXVMTimeout(), s.b.RPCGasCap())
	if err != nil {
		return nil, err
	}
...
// ExecutionResult groups all structured logs emitted by the XVM
...
// StructLogRes stores a structured log emitted by the XVM while replaying a
...
// FormatLogs formats XVM returned structured logs for json output
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		vmenv, _, err := b.GetXVM(ctx, msg, statedb, header, &config)
		if err != nil {
			return nil, 0, nil, err
		}
...
//
// https://github.com/xpayments/wiki/wiki/JSON-RPC#xps_sign
...
// PublicDebugAPI is the collection of xPayments APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", xpsash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of xPayments APIs exposed over the private
// debugging endpoint.
...
// Version returns the current xpayments protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeXps := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Xps)))
	feeFloat, _ := feeXps.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f xps) exceeds the configured cap (%.2f xps)", feeFloat, cap)
	}
	return nil
}
