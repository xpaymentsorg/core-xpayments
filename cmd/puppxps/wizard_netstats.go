...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for xpsstats availability")
	if infos, err := checkXpsstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["xpsstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["xpsstats"] = infos.Report()
		xpsstats = infos.config
	}
