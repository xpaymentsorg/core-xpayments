...
func (w *wizard) makeGenesis() {
	...
	fmt.Println(" 1. Xpsash - proof-of-work")
	fmt.Println(" 2. Clique - proof-of-authority")

	choice := w.read()
	switch {
	case choice == "1":
		// In case of xpsash, we're pretty much done
		genesis.Config.Xpsash = new(params.XpsashConfig)
...
// importGenesis imports a Gpay genesis spec into puppxps.
...
func (w *wizard) manageGenesis() {
	// Figure out whxpser to modify or export the genesis
	fmt.Println()
	...
	case "2":
		...
		fmt.Printf("  Will create %s.json, %s-alpay.json, %s-harmony.json, %s-parity.json\n", w.network, w.network, w.network, w.network)
		...
		// Export the native genesis spec used by puppxps and Gpay
		gpayJson := filepath.Join(folder, fmt.Sprintf("%s.json", w.network))
		if err := ioutil.WriteFile(gpayJson, out, 0644); err != nil {
			log.Error("Failed to save genesis file", "err", err)
			return
		}
		log.Info("Saved native genesis chain spec", "path", gpayJson)

		// Export the genesis spec used by Alpay (formerly C++ xPayments)
		if spec, err := newAlpayGenesisSpec(w.network, w.conf.Genesis); err != nil {
			log.Error("Failed to create Alpay chain spec", "err", err)
		} else {
			saveGenesis(folder, w.network, "alpay", spec)
		}
		...
		// Export the genesis spec used by Harmony (formerly xPaymentsJ)
