...
// makeWizard creates and returns a new puppxps wizard.
...
// setting up a new or managing an existing xPayments private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppxps, your xPayments private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new xPayments network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and xpsstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppxps uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering xPayments network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppxps", w.network)
