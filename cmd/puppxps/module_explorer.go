...
FROM puppxps/blockscout:latest

ADD genesis.json /genesis.json
RUN \
  echo 'gpay --cache 512 init /genesis.json' > explorer.sh && \
  echo $'gpay --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.XpsPort}} --bootnodes {{.Bootnodes}} --xpsstats \'{{.Xpsstats}}\' --cache=512 --http --http.api "net,web3,xps,debug,txpool" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" --exitwhensynced' >> explorer.sh && \
  echo $'exec gpay --networkid {{.NetworkID}} --syncmode "full" --gcmode "archive" --port {{.XpsPort}} --bootnodes {{.Bootnodes}} --xpsstats \'{{.Xpsstats}}\' --cache=512 --http --http.api "net,web3,xps,debug,txpool" --http.corsdomain "*" --http.vhosts "*" --ws --ws.origins "*" &' >> explorer.sh && \
...
services:
    explorer:
        ...
        ports:
            - "{{.XpsPort}}:{{.XpsPort}}"
            - "{{.XpsPort}}:{{.XpsPort}}/udp"{{if not .VHost}}
            - "{{.WebPort}}:4000"{{end}}
        environment:
            - XPS_PORT={{.XpsPort}}
            - XPS_NAME={{.XpsName}}
...
func deployExplorer(client *sshClient, network string, bootnodes []string, config *explorerInfos, nocache bool, isClique bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(explorerDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Xpsstats":  config.node.xpsstats,
		"XpsPort":   config.node.port,
	})
	...
	template.Must(template.New("").Parse(explorerComposefile)).Execute(composefile, map[string]interface{}{
		...
		"DBDir":       config.dbdir,
		"XpsPort":     config.node.port,
		"XpsName":     config.node.xpsstats[:strings.Index(config.node.xpsstats, ":")],
		"WebPort":     config.port,
...
func (info *explorerInfos) Report() map[string]string {
	report := map[string]string{
		...
		"xPayments listener port ": strconv.Itoa(info.node.port),
		"Xpsstats username":        info.node.xpsstats,
	}
	return report
}
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	p2pPort := infos.portmap[infos.envvars["XPS_PORT"]+"/tcp"]
	...
	stats := &explorerInfos{
		node: &nodeInfos{
			datadir:  infos.volumes["/opt/app/.xpayments"],
			port:     infos.portmap[infos.envvars["XPS_PORT"]+"/tcp"],
			xpsstats: infos.envvars["XPS_NAME"],
		},
