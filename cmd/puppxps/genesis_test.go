...
// Tests the go-xpayments to Alpay chainspec conversion for the Stureby testnet.
func TestAlpaySturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gpay.json")
	if err != nil {
	...
	spec, err := newAlpayGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alpay.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alpayGenesisSpec{}
...
// Tests the go-xpayments to Parity chainspec conversion for the Stureby testnet.
