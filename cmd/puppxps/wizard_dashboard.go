...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"xpsstats", "explorer", "faucet"} {
		...
		for _, server := range available[service] {
			...
			switch service {
			case "xpsstats":
				if infos, err := checkXpsstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have xpsstats running, ask whether to make the secret public or not
	if w.conf.xpsstats != "" {
		fmt.Println()
		fmt.Println("Include xpsstats secret on dashboard (y/n)? (default = yes)")
