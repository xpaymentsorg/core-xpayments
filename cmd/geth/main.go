...
func init() {
	...
	// // add bor flags
	// app.Flags = append(app.Flags, utils.XPoSFlag...)
...
func prepare(ctx *cli.Context) {
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.GlobalIsSet(utils.RopstenFlag.Name) && !ctx.GlobalIsSet(utils.RinkebyFlag.Name) && !ctx.GlobalIsSet(utils.GoerliFlag.Name) && !ctx.GlobalIsSet(utils.DeveloperFlag.Name) { // !ctx.GlobalIsSet(utils.MumbaiFlag.Name) &&
