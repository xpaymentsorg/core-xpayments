...
// NewSuite creates and returns a new xps-test suite that can
...
func (s *Suite) AllXpsTests() []utesting.Test {
...
func (s *Suite) XpsTests() []utesting.Test {
	return []utesting.Test{
		{Name: "TestStatus65", Fn: s.TestStatus65},
		{Name: "TestGetBlockHeaders65", Fn: s.TestGetBlockHeaders65},
		{Name: "TestGetBlockBodies65", Fn: s.TestGetBlockBodies65},
...
func (s *Suite) Xps66Tests() []utesting.Test {
	return []utesting.Test{
		// only proceed with xps66 test suite if node supports xps 66 protocol
		{Name: "TestStatus66", Fn: s.TestStatus66},
		{Name: "TestGetBlockHeaders66", Fn: s.TestGetBlockHeaders66},
		{Name: "TestSimultaneousRequests66", Fn: s.TestSimultaneousRequests66},
		{Name: "TestSameRequestID66", Fn: s.TestSameRequestID66},
...
var (
	xps66 = true  // indicates whether suite should negotiate xps66 connection
	xps65 = false // indicates whether suite should negotiate xps65 connection or below.
)
...
// a status message with it on the xps66 protocol.
...
// an xps66 `GetBlockHeaders` request and that the response is accurate.
func (s *Suite) TestGetBlockHeaders66(t *utesting.T) {
	...
	headers, err := conn.headersRequest(req, s.chain, xps66, 33)
	if err != nil {
...
func (s *Suite) TestSimultaneousRequests66(t *utesting.T) {
	...
	// create two requests
	req1 := &xps.GetBlockHeadersPacket66{
		RequestId: uint64(111),
		GetBlockHeadersPacket: &xps.GetBlockHeadersPacket{
			Origin: xps.HashOrNumber{
				Hash: s.chain.blocks[1].Hash(),
			},
			Amount:  2,
	...
	req2 := &xps.GetBlockHeadersPacket66{
...
func (s *Suite) TestSameRequestID66(t *utesting.T) {
	...
	// create requests
	reqID := uint64(1234)
	request1 := &xps.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &xps.GetBlockHeadersPacket{
			Origin: xps.HashOrNumber{
				Number: 1,
	...
	request2 := &xps.GetBlockHeadersPacket66{
		RequestId: reqID,
		GetBlockHeadersPacket: &xps.GetBlockHeadersPacket{
			Origin: xps.HashOrNumber{
				Number: 33,
...
func (s *Suite) TestZeroRequestID66(t *utesting.T) {
	...
	headers, err := conn.headersRequest(req, s.chain, xps66, 0)
	if err != nil {
...
func (s *Suite) TestGetBlockBodies66(t *utesting.T) {
	...
	// create block bodies request
	req := &xps.GetBlockBodiesPacket66{
		RequestId: uint64(55),
		GetBlockBodiesPacket: xps.GetBlockBodiesPacket{
			s.chain.blocks[54].Hash(),
			s.chain.blocks[75].Hash(),
		},
	}
...
func (s *Suite) TestBlockHashAnnounce65(t *utesting.T) {
	if err := s.hashAnnounce(xps65); err != nil {
...
func (s *Suite) TestBlockHashAnnounce66(t *utesting.T) {
	if err := s.hashAnnounce(xps66); err != nil {
		t.Fatalf("block hash announcement failed: %v", err)
	}
}
...
func (s *Suite) TestMaliciousHandshake65(t *utesting.T) {
	if err := s.maliciousHandshakes(t, xps65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousHandshake66(t *utesting.T) {
	if err := s.maliciousHandshakes(t, xps66); err != nil {
		t.Fatal(err)
	}
}
...
// difficulty over the xps66 protocol.
...
func (s *Suite) TestTransaction65(t *utesting.T) {
	if err := s.sendSuccessfulTxs(t, xps65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestTransaction66(t *utesting.T) {
	if err := s.sendSuccessfulTxs(t, xps66); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousTx65(t *utesting.T) {
	if err := s.sendMaliciousTxs(t, xps65); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestMaliciousTx66(t *utesting.T) {
	if err := s.sendMaliciousTxs(t, xps66); err != nil {
		t.Fatal(err)
	}
}
...
func (s *Suite) TestLargeTxRequest66(t *utesting.T) {
	...
	getTxReq := &xps.GetPooledTransactionsPacket66{
		RequestId:                   1234,
