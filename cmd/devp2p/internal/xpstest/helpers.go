...
// Is_66 checks if the node supports the xps66 protocol version,
...
// returning the created Conn with additional xps66 capabilities if
// successful
func (s *Suite) dial66() (*Conn, error) {
	...
	conn.caps = append(conn.caps, p2p.Cap{Name: "xps", Version: 66})
...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		c.negotiateXpsProtocol(msg.Caps)
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("could not negotiate xps protocol (remote caps: %v, local xps version: %v)", msg.Caps, c.ourHighestProtoVersion)
		}
...
// negotiateXpsProtocol sets the Conn's xps protocol version to highest
// advertised capability from peer.
func (c *Conn) negotiateXpsProtocol(caps []p2p.Cap) {
	var highestXpsVersion uint
	var highestSnapVersion uint
	for _, capability := range caps {
		switch capability.Name {
		case "xps":
			if capability.Version > highestXpsVersion && capability.Version <= c.ourHighestProtoVersion {
				highestXpsVersion = capability.Version
			}
		case "snap":
	...
	c.negotiatedProtoVersion = highestXpsVersion
...
loop:
	...
	// make sure xps protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("xps protocol version must be set in Conn")
	}
	if status == nil {
...
func (s *Suite) createSendAndRecvConns(isXps66 bool) (*Conn, *Conn, error) {
	var (
		sendConn *Conn
		recvConn *Conn
		err      error
...
// readAndServe66 serves xps66 GetBlockHeaders requests while waiting
// on another message from the node.
func (c *Conn) readAndServe66(chain *Chain, timeout time.Duration) (uint64, Message) {
	...
	for time.Since(start) < timeout {
		...
		case GetBlockHeaders:
			...
			resp := &xps.BlockHeadersPacket66{
				RequestId:          reqID,
				BlockHeadersPacket: xps.BlockHeadersPacket(headers),
			}
...
func (c *Conn) headersRequest(request *GetBlockHeaders, chain *Chain, isXps66 bool, reqID uint64) (BlockHeaders, error) {
	defer c.SetReadDeadline(time.Time{})
	c.SetReadDeadline(time.Now().Add(20 * time.Second))
	// if on xps66 connection, perform xps66 GetBlockHeaders request
	if isXps66 {
...
// getBlockHeaders66 executes the given `GetBlockHeaders` request over the xps66 protocol.
func getBlockHeaders66(chain *Chain, conn *Conn, request *GetBlockHeaders, id uint64) (BlockHeaders, error) {
	// write request
	packet := xps.GetBlockHeadersPacket(*request)
	req := &xps.GetBlockHeadersPacket66{
		RequestId:             id,
...
func (s *Suite) sendNextBlock(isXps66 bool) error {
	// set up sending and receiving connections
	sendConn, recvConn, err := s.createSendAndRecvConns(isXps66)
	if err != nil {
		return err
	}
	...
	if err = s.waitForBlockImport(recvConn, nextBlock, isXps66); err != nil {
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block, isXps66 bool) error {
	...
	for {
		...
		if isXps66 {
			requestID := uint64(54)
			headers, err = conn.headersRequest(req, s.chain, xps66, requestID)
		} else {
			headers, err = conn.headersRequest(req, s.chain, xps65, 0)
		}
		if err != nil {
...
func (s *Suite) oldAnnounce(isXps66 bool) error {
	sendConn, receiveConn, err := s.createSendAndRecvConns(isXps66)
	if err != nil {
		return err
	}
	defer sendConn.Close()
...
func (s *Suite) maliciousHandshakes(t *utesting.T, isXps66 bool) error {
	var (
		conn *Conn
		err  error
	)
...
func (s *Suite) hashAnnounce(isXps66 bool) error {
	// create connections
	sendConn, recvConn, err := s.createSendAndRecvConns(isXps66)
	if err != nil {
	...
	if isXps66 {
		...
		if err := sendConn.Write66(&xps.BlockHeadersPacket66{
			RequestId: id,
			BlockHeadersPacket: xps.BlockHeadersPacket{
				nextBlock.Header(),
			},
