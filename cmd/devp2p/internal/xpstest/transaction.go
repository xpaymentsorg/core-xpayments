...
func (s *Suite) sendSuccessfulTxs(t *utesting.T, isXps66 bool) error {
	...
	for i, tx := range tests {
		...
		if err := sendSuccessfulTx(s, tx, prevTx, isXps66); err != nil {
...
func sendSuccessfulTx(s *Suite, tx *types.Transaction, prevTx *types.Transaction, isXps66 bool) error {
...
func (s *Suite) sendMaliciousTxs(t *utesting.T, isXps66 bool) error {
	...
	for i, tx := range badTxs {
		...
		if err = sendMaliciousTx(s, tx, isXps66); err != nil {
...
func sendMaliciousTx(s *Suite, tx *types.Transaction, isXps66 bool) error {
	// setup connection
	var (
		conn *Conn
		err  error
