...
func removeDB(ctx *cli.Context) error {
	...
	path = config.Xps.DatabaseFreezer
	switch {
	case path == "":
...
func showLeveldbStats(db xpsdb.Stater) {
...
type snapshotIterator struct {
	init    bool
	account xpsdb.Iterator
	storage xpsdb.Iterator
}
...
var chainExporters = map[string]func(db xpsdb.Database) utils.ChainDataIterator{
	"preimage": func(db xpsdb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db xpsdb.Database) utils.ChainDataIterator {
...
func dbHasLegacyReceipts(db xpsdb.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
