...
func remoteConsole(ctx *cli.Context) error {
	// Attach to a remotely running gpay instance and start the JavaScript console
	...
	if endpoint == "" {
		...
		if path != "" {
			...
			} else if ctx.GlobalBool(utils.BerylliumFlag.Name) || ctx.GlobalBool(utils.XPSMainnetFlag.Name) {
				homeDir, _ := os.UserHomeDir()
				path = filepath.Join(homeDir, "/.xpayments/data")
			}
		}
