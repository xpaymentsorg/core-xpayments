...
const (
	...
	repositoryIdentifier = "go-xpayments"
)
...
func init() {
	...
	// add xpos flags
	app.Flags = append(app.Flags, utils.XPoSFlags...)
...
func prepare(ctx *cli.Context) {
	...
	switch {
	case ctx.GlobalIsSet(utils.BerylliumFlag.Name):
		log.Info("Starting XPS on Beryllium testnet...")

	case ctx.GlobalIsSet(utils.XPSMainnetFlag.Name):
		log.Info("Starting XPS on xPayments mainnet...")
	}

	// If we're a full node on mainnet without --cache specified, bump default cache allowance
	if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
		// Make sure we're not on any supported preconfigured testnet either
		if !ctx.GlobalIsSet(utils.RopstenFlag.Name) && !ctx.GlobalIsSet(utils.RinkebyFlag.Name) && !ctx.GlobalIsSet(utils.GoerliFlag.Name) && !ctx.GlobalIsSet(utils.BerylliumFlag.Name) && !ctx.GlobalIsSet(utils.DeveloperFlag.Name) {
