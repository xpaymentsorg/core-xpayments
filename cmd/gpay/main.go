...
import (
	...
	_ "github.com/xpaymentsorg/go-xpayments/xps/tracers/native"
...
var (
	...
	nodeFlags = []cli.Flag{
		...
		utils.XpsashCacheDirFlag,
		utils.XpsashCachesInMemoryFlag,
		utils.XpsashCachesOnDiskFlag,
		utils.XpsashCachesLockMmapFlag,
		utils.XpsashDatasetDirFlag,
		utils.XpsashDatasetsInMemoryFlag,
		utils.XpsashDatasetsOnDiskFlag,
		utils.XpsashDatasetsLockMmapFlag,
		...
		utils.XpsPeerRequiredBlocksFlag,
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend xpsapi.Backend, isConsole bool) {
	...
	xpsClient := xpsclient.NewClient(rpcClient)
	...
	go func() {
		...
		for event := range events {
			...
			case accounts.WalletOpened:
				...
				event.Wallet.SelfDerive(derivationPaths, xpsClient)
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		xpsBackend, ok := backend.(*xps.XpsAPIBackend)
		if !ok {
			utils.Fatalf("xPayments service not running")
		}
		// Set the gas price to the limits from the CLI and start mining
		gasprice := utils.GlobalBig(ctx, utils.MinerGasPriceFlag.Name)
		xpsBackend.TxPool().SetGasPrice(gasprice)
		// start mining
		threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name)
		if err := xpsBackend.StartMining(threads); err != nil {
