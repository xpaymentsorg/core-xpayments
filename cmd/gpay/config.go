...
func makeConfigNode(ctx *cli.Context) (*node.Node, gpayConfig) {
	...
	if ctx.GlobalIsSet(utils.BerylliumFlag.Name) {
		setDefaultBerylliumGpayConfig(ctx, &cfg)
	}

	if ctx.GlobalIsSet(utils.XPSMainnetFlag.Name) {
		setDefaultXPSMainnetGpayConfig(ctx, &cfg)
	}
	...
	// Set XPS config flags
	utils.SetXPoSConfig(ctx, &cfg.Eth)
...
func setDefaultBerylliumGpayConfig(ctx *cli.Context, config *gpayConfig) {
	config.Node.P2P.ListenAddr = fmt.Sprintf(":%d", 30450)
	config.Node.HTTPHost = "0.0.0.0"
	config.Node.HTTPVirtualHosts = []string{"*"}
	config.Node.HTTPCors = []string{"*"}
	config.Node.HTTPPort = 8450
	config.Node.IPCPath = utils.MakeDataDir(ctx) + "/gpay.ipc"
	config.Node.HTTPModules = []string{"eth", "net", "web3", "txpool", "gpay"}
	config.Eth.SyncMode = downloader.FullSync
	config.Eth.NetworkId = 34324
...
func setDefaultXPSMainnetGpayConfig(ctx *cli.Context, config *gpayConfig) {
	config.Node.P2P.ListenAddr = fmt.Sprintf(":%d", 30350)
	config.Node.HTTPHost = "0.0.0.0"
	config.Node.HTTPVirtualHosts = []string{"*"}
	config.Node.HTTPCors = []string{"*"}
	config.Node.HTTPPort = 8550
	config.Node.IPCPath = utils.MakeDataDir(ctx) + "/gpay.ipc"
	config.Node.HTTPModules = []string{"eth", "net", "web3", "txpool", "gpay"}
	config.Eth.SyncMode = downloader.FullSync
	config.Eth.NetworkId = 34323
	config.Eth.Miner.GasCeil = 20000000
	//--miner.gastarget is depreceated, No longed used
	config.Eth.TxPool.NoLocals = true
	config.Eth.TxPool.AccountSlots = 16
