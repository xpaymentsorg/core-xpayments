...
type xpsstatsConfig struct {
	URL string `toml:",omitempty"`
}

type gpayConfig struct {
	Xps      xpsconfig.Config
	Node     node.Config
	Xpsstats xpsstatsConfig
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "xps")
	cfg.WSModules = append(cfg.WSModules, "xps")
...
func makeConfigNode(ctx *cli.Context) (*node.Node, gpayConfig) {
	...
	if ctx.GlobalIsSet(utils.XpsStatsURLFlag.Name) {
		cfg.Xpsstats.URL = ctx.GlobalString(utils.XpsStatsURLFlag.Name)
	}
...
func makeFullNode(ctx *cli.Context) (*node.Node, xpsapi.Backend) {
	stack, cfg := makeConfigNode(ctx)
	if ctx.GlobalIsSet(utils.OverrideArrowGlacierFlag.Name) {
		cfg.Xps.OverrideArrowGlacier = new(big.Int).SetUint64(ctx.GlobalUint64(utils.OverrideArrowGlacierFlag.Name))
	}
	if ctx.GlobalIsSet(utils.OverrideTerminalTotalDifficulty.Name) {
		cfg.Xps.OverrideTerminalTotalDifficulty = new(big.Int).SetUint64(ctx.GlobalUint64(utils.OverrideTerminalTotalDifficulty.Name))
	}
	backend, xps := utils.RegisterXpsService(stack, &cfg.Xps)
	// Warn users to migrate if they have a legacy freezer format.
	if xps != nil {
		firstIdx := uint64(0)
		// Hack to speed up check for mainnet because we know
		// the first non-empty block.
		ghash := rawdb.ReadCanonicalHash(xps.ChainDb(), 0)
		if cfg.Xps.NetworkId == 1 && ghash == params.MainnetGenesisHash {
			firstIdx = 46147
		}
		isLegacy, _, err := dbHasLegacyReceipts(xps.ChainDb(), firstIdx)
		if err != nil {
	...
	if cfg.Xpsstats.URL != "" {
		utils.RegisterXpsStatsService(stack, backend, cfg.Xpsstats.URL)
	}
...
func deprecated(field string) bool {
	switch field {
	case "xpsconfig.Config.XVMInterpreter":
		return true
	case "xpsconfig.Config.EWASMInterpreter":
		return true
	default:
		return false
	}
