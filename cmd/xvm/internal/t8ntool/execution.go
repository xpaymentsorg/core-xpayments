...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig,
	...
	getTracerFn func(txIndex int, txHash common.Hash) (tracer vm.XVMLogger, err error)) (*state.StateDB, *ExecutionResult, error) {
	...
	// If DAO is supported/enabled, we need to handle it here. In gpay 'proper', it's
	...
	for i, tx := range txs {
		...
		xvm := vm.NewXVM(vmContext, txContext, statedb, chainConfig, vmConfig)

		// (ret []byte, usedGas uint64, failed bool, err error)
		msgResult, err := core.ApplyMessage(xvm, msg, gaspool)
		if err != nil {
	...
	if err != nil {
		...
		return nil, nil, NewError(ErrorXVM, fmt.Errorf("could not commit state: %v", err))
	}
...
func MakePreState(db xpsdb.Database, accounts core.GenesisAlloc) *state.StateDB {
...
// calcDifficulty is based on xpsash.CalcDifficulty. This method is used in case
// the caller does not provide an explicit difficulty, but instead provides only
// parent timestamp + difficulty.
// Note: this method only works for xpsash engine.
func calcDifficulty(config *params.ChainConfig, number, currentTime, parentTime uint64,
	...
	return xpsash.CalcDifficulty(config, currentTime, parent)
}
