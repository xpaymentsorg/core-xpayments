...
func (db *Database) NewBatch() xpsdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewBatchWithSize(size int) xpsdb.Batch {
	return &batch{
		db: db.db,
...
func (db *Database) NewIterator(prefix []byte, start []byte) xpsdb.Iterator {
...
func (db *Database) NewSnapshot() (xpsdb.Snapshot, error) {
...
func (b *batch) Replay(w xpsdb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  xpsdb.KeyValueWriter
	failure error
}
