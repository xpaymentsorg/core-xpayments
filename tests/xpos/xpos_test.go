...
import (
	...
	"github.com/xpaymentsorg/go-xpayments/tests/xpos/mocks"
	"golang.org/x/crypto/sha3"
)

var (
	spanPath         = "xpos/span/1"
...
func TestInsertingSpanSizeBlocks(t *testing.T) {
	...
	_xpos := engine.(*xpos.XPoS)
	h, genisysSpan := getMockedGenisysClient(t)
	_xpos.SetGenisysClient(h)
	...
	for i := uint64(1); i <= spanSize; i++ {
		block = buildNextBlock(t, _xpos, chain, block, nil, init.genesis.Config.XPoS)
		insertNewBlock(t, chain, block)
	}

	assert.True(t, h.AssertCalled(t, "FetchWithRetry", spanPath, ""))
	validators, err := _xpos.GetCurrentValidators(block.Hash(), spanSize) // check validator set at the first block of new span
	if err != nil {
		t.Fatalf("%s", err)
	}
	...
	for i, validator := range validators {
		assert.Equal(t, validator.Address.Bytes(), genisysSpan.SelectedProducers[i].Address.Bytes())
		assert.Equal(t, validator.VotingPower, genisysSpan.SelectedProducers[i].VotingPower)
	}
}
...
func TestFetchStateSyncEvents(t *testing.T) {
	...
	// B. Before inserting 1st block of the next sprint, mock genisys deps
	// B.1 Mock /xpos/span/1
	res, _ := loadSpanFromFile(t)
	h := &mocks.IGenisysClient{}
...
func TestFetchStateSyncEvents_2(t *testing.T) {
	...
	eventRecords := []*xpos.EventRecordWithTime{
	...
	assert.True(t, h.AssertCalled(t, "FetchWithRetry", spanPath, ""))
	assert.True(t, h.AssertCalled(t, "FetchStateSyncEvents", fromID, to))
	lastStateID, _ := _xpos.GenesisContractsClient.LastStateId(sprintSize)
	...
	assert.True(t, h.AssertCalled(t, "FetchStateSyncEvents", fromID, to))
	lastStateID, _ = _xpos.GenesisContractsClient.LastStateId(spanSize)
...
func TestOutOfTurnSigning(t *testing.T) {
	...
	block = buildNextBlock(t, _xpos, chain, block, signerKey, init.genesis.Config.XPoS)
	_, err := chain.InsertChain([]*types.Block{block})
	assert.Equal(t,
		*err.(*xpos.BlockTooSoonError),
		xpos.BlockTooSoonError{Number: spanSize, Succession: expectedSuccessionNumber})

	expectedDifficulty := uint64(3 - expectedSuccessionNumber) // len(validators) - succession
	header := block.Header()
	header.Time += (xpos.CalcProducerDelay(header.Number.Uint64(), expectedSuccessionNumber, init.genesis.Config.XPoS) -
		xpos.CalcProducerDelay(header.Number.Uint64(), 0, init.genesis.Config.XPoS))
	sign(t, header, signerKey, init.genesis.Config.XPoS)
	block = types.NewBlockWithHeader(header)
	_, err = chain.InsertChain([]*types.Block{block})
	assert.Equal(t,
		*err.(*xpos.WrongDifficultyError),
		xpos.WrongDifficultyError{Number: spanSize, Expected: expectedDifficulty, Actual: 3, Signer: addr.Bytes()})
...
func TestSignerNotFound(t *testing.T) {
	...
	_, err := chain.InsertChain([]*types.Block{block})
	assert.Equal(t,
		*err.(*xpos.UnauthorizedSignerError),
		xpos.UnauthorizedSignerError{Number: 0, Signer: addr.Bytes()})
}

func getMockedGenisysClient(t *testing.T) (*mocks.IGenisysClient, *xpos.GenisysSpan) {
	res, genisysSpan := loadSpanFromFile(t)
	h := &mocks.IGenisysClient{}
	h.On("FetchWithRetry", "xpos/span/1", "").Return(res, nil)
	h.On(
		"FetchStateSyncEvents",
		mock.AnythingOfType("uint64"),
		mock.AnythingOfType("int64")).Return([]*xpos.EventRecordWithTime{getSampleEventRecord(t)}, nil)
	return h, genisysSpan
}

func generateFakeStateSyncEvents(sample *xpos.EventRecordWithTime, count int) []*xpos.EventRecordWithTime {
	events := make([]*xpos.EventRecordWithTime, count)
	event := *sample
	event.ID = 1
	events[0] = &xpos.EventRecordWithTime{}
	*events[0] = event
	for i := 1; i < count; i++ {
		event.ID = uint64(i)
		event.Time = event.Time.Add(1 * time.Second)
		events[i] = &xpos.EventRecordWithTime{}
		*events[i] = event
	}
	return events
}

func buildStateEvent(sample *xpos.EventRecordWithTime, id uint64, timeStamp int64) *xpos.EventRecordWithTime {
	event := *sample
	event.ID = id
...
func getSampleEventRecord(t *testing.T) *xpos.EventRecordWithTime {
	res := stateSyncEventsPayload(t)
	var _eventRecords []*xpos.EventRecordWithTime
...
func TestEIP1559Transition(t *testing.T) {
	var (
		...
		gspec = &core.Genesis{
			Config: params.XPSTestChainConfig,
	...
	actual = state.GetBalance(common.HexToAddress(params.XPSTestChainConfig.XPoS.CalculateBurntContract(block.NumberU64())))
...
func TestJaipurFork(t *testing.T) {
	...
	for i := uint64(1); i < sprintSize; i++ {
		block = buildNextBlock(t, _xpos, chain, block, nil, init.genesis.Config.XPoS)
		insertNewBlock(t, chain, block)
		if block.Number().Uint64() == init.genesis.Config.XPoS.JaipurBlock-1 {
			assert.Equal(t, testSealHash(block.Header(), init.genesis.Config.XPoS), xpos.SealHash(block.Header(), init.genesis.Config.XPoS))
		}
		if block.Number().Uint64() == init.genesis.Config.XPoS.JaipurBlock {
			assert.Equal(t, testSealHash(block.Header(), init.genesis.Config.XPoS), xpos.SealHash(block.Header(), init.genesis.Config.XPoS))
		}
	}
}
...
func testSealHash(header *types.Header, c *params.XPoSConfig) (hash common.Hash) {
...
func testEncodeSigHeader(w io.Writer, header *types.Header, c *params.XPoSConfig) {
	enc := []interface{}{
		header.ParentHash,
		header.UncleHash,
		header.Coinbase,
