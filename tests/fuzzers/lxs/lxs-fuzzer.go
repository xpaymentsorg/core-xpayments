...
import (
	...
	l "github.com/xpaymentsorg/go-xpayments/lxs"
...
var (
	...
	bankFunds  = new(big.Int).Mul(big.NewInt(100), big.NewInt(params.Xps))
...
func makechain() (bc *core.BlockChain, addrHashes, txHashes []common.Hash) {
	...
	blocks, _ := core.GenerateChain(gspec.Config, genesis, xpsash.NewFaker(), db, testChainLen,
	...
	bc, _ = core.NewBlockChain(db, nil, gspec.Config, xpsash.NewFaker(), vm.Config{}, nil, nil)
...
func (f *fuzzer) doFuzz(msgCode uint64, packet interface{}) {
	...
	defer closeFn()
	fn, _, _, err := l.Lxs3[msgCode].Handle(dummyMsg{enc})
	if err != nil {
		panic(err)
	}
