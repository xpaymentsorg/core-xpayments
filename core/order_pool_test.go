...
func getNonce(t *testing.T, userAddress common.Address) (uint64, error) {
	...
	err = rpcClient.Call(&result, "XPSx_getOrderCount", userAddress)
	if err != nil {
		return 0, err
	}
	s := result.(string)
...
func testSendOrder(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
	...
	msg := &OrderMsg{
		...
		BaseToken:       common.HexToAddress(common.XPSNativeAddress),
...
func testSendOrderXPSUSD(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func testSendOrderXPSBTC(t *testing.T, amount, price *big.Int, side string, status string, orderID uint64) {
...
func TestFilled(t *testing.T) {
	...
	//testSendOrderXPSUSD(t, new(big.Int).Mul(big.NewInt(1000000000000000000), big.NewInt(5000)), BTCUSDPrice, "BUY", "NEW", 0)
	//ETH/BTC
	...
	XPSBTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	time.Sleep(2 * time.Second)
	testSendOrderXPSBTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSBTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderXPSBTC(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSBTCPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderXPSBTC(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), XPSBTCPrice, "SELL", "NEW", 0)

	XPSUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(6)) // 0.6
	time.Sleep(2 * time.Second)
	testSendOrderXPSUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderXPSUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderXPSUSD(t, new(big.Int).Mul(big.NewInt(1200000), _1E18), XPSUSDPrice, "SELL", "NEW", 0)

}

func TestX10Filled(t *testing.T) {
	XPSUSDPrice := new(big.Int).Mul(_1E7, big.NewInt(60)) // 6
	time.Sleep(2 * time.Second)
	testSendOrderXPSUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSUSDPrice, "BUY", "NEW", 0)
	time.Sleep(2 * time.Second)
	testSendOrderXPSUSD(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSUSDPrice, "BUY", "NEW", 0)
...
func TestCancelOrder(t *testing.T) {
	XPSBTCPrice := new(big.Int).Mul(big.NewInt(10000000000000), big.NewInt(6)) // 0.00006
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSBTCPrice, "BUY", "NEW", 0)
	time.Sleep(5 * time.Second)
	testSendOrder(t, new(big.Int).Mul(big.NewInt(600000), _1E18), XPSBTCPrice, "BUY", "CANCELLED", 3)
	time.Sleep(5 * time.Second)
