...
// blockChain_XPSx add order state
...
func (pool *LendingPool) reset(oldHead, newblock *types.Block) {
	if !pool.chainconfig.IsTIPXPSX(pool.chain.CurrentBlock().Number()) || pool.chain.Config().XPoS == nil || pool.chain.CurrentBlock().NumberU64() <= pool.chain.Config().XPoS.Epoch {
		return
	}
...
func (pool *LendingPool) validateBalance(cloneStateDb *state.StateDB, cloneLendingStateDb *lendingstate.LendingStateDB, tx *types.LendingTransaction, collateralToken common.Address) error {
	XPoSEngine, ok := pool.chain.Engine().(*XPoS.XPoS)
	if !ok {
		return ErrNotXPoS
	}
	XPSXServ := XPoSEngine.GetXPSXService()
	lendingServ := XPoSEngine.GetLendingService()
	if XPSXServ == nil {
		return fmt.Errorf("XPSx not found in order validation")
	}
	lendingTokenDecimal, err := XPSXServ.GetTokenDecimal(pool.chain, cloneStateDb, tx.LendingToken())
	if err != nil {
	...
	tradingStateDb, err := XPSXServ.GetTradingState(pool.chain.CurrentBlock(), author)
	if err != nil {
	...
	// lendTokenXPSPrice: price of lendingToken in XPS quote
	var lendTokenXPSPrice, collateralPrice, collateralTokenDecimal *big.Int
	if collateralToken.String() != lendingstate.EmptyAddress {
		collateralTokenDecimal, err = XPSXServ.GetTokenDecimal(pool.chain, cloneStateDb, collateralToken)
		if err != nil {
			return fmt.Errorf("validateOrder: failed to get collateralTokenDecimal. err: %v", err)
		}
		lendTokenXPSPrice, collateralPrice, err = lendingServ.GetCollateralPrices(pool.chain.CurrentHeader(), pool.chain, cloneStateDb, cloneTradingStateDb, collateralToken, tx.LendingToken())
		if err != nil {
			return err
		}
		if lendTokenXPSPrice == nil || lendTokenXPSPrice.Sign() <= 0 || collateralPrice == nil || collateralPrice.Sign() <= 0 {
			log.Debug("ValidateLending: ErrInvalidCollateralPrice", "lendTokenXPSPrice", lendTokenXPSPrice, "collateralPrice", collateralPrice)
			return lendingstate.ErrInvalidCollateralPrice
		}
	}
	if lendTokenXPSPrice == nil || lendTokenXPSPrice.Sign() == 0 {
		if tx.LendingToken().String() == common.XPSNativeAddress {
			lendTokenXPSPrice = common.BasePrice
		} else {
			lendTokenXPSPrice, err = lendingServ.GetMediumTradePriceBeforeEpoch(pool.chain, cloneStateDb, cloneTradingStateDb, tx.LendingToken(), common.HexToAddress(common.XPSNativeAddress))
			if err != nil {
				return err
			}
		}
	}
	isXPSXLendingFork := pool.chain.Config().IsTIPXPSXLending(pool.chain.CurrentHeader().Number)
	if err := lendingstate.VerifyBalance(isXPSXLendingFork,
		cloneStateDb,
		cloneLendingStateDb,
		tx.Type(),
		tx.Side(),
...
func (pool *LendingPool) addTx(tx *types.LendingTransaction, local bool) error {
	if !pool.chainconfig.IsTIPXPSX(pool.chain.CurrentBlock().Number()) {
		return nil
	}
	tx.CacheHash()
