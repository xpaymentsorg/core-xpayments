...
func NewBlockChainEx(db ethdb.Database, XPSxDb ethdb.XPSxDatabase, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	blockchain, err := NewBlockChain(db, cacheConfig, chainConfig, engine, vmConfig)
	if err != nil {
		return nil, err
	}
	if blockchain != nil {
		blockchain.addXPSxDb(XPSxDb)
	}
...
func (bc *BlockChain) addXPSxDb(XPSxDb ethdb.XPSxDatabase) {
	bc.XPSxDb = XPSxDb
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if err != nil {
		...
		engine, ok := bc.Engine().(*XPoS.XPoS)
		if ok {
			tradingService := engine.GetXPSXService()
			lendingService := engine.GetLendingService()
			if bc.Config().IsTIPXPSX(currentBlock.Number()) && bc.chainConfig.XPoS != nil && currentBlock.NumberU64() > bc.chainConfig.XPoS.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) OrderStateAt(block *types.Block) (*tradingstate.TradingStateDB, error) {
	engine, ok := bc.Engine().(*XPoS.XPoS)
	if ok {
		XPSXService := engine.GetXPSXService()
		if bc.Config().IsTIPXPSX(block.Number()) && bc.chainConfig.XPoS != nil && block.NumberU64() > bc.chainConfig.XPoS.Epoch && XPSXService != nil {
			...
			if err == nil {
				return XPSxState, nil
			} else {
				return nil, err
			}
		} else {
			XPSxState, err := XPSXService.GetEmptyTradingState()
			if err == nil {
				return XPSxState, nil
			} else {
				return nil, err
	...
	return nil, errors.New("Get XPSx state fail")
...
func (bc *BlockChain) LendingStateAt(block *types.Block) (*lendingstate.LendingStateDB, error) {
	...
	if ok {
		lendingService := engine.GetLendingService()
		if bc.Config().IsTIPXPSX(block.Number()) && bc.chainConfig.XPoS != nil && block.NumberU64() > bc.chainConfig.XPoS.Epoch && lendingService != nil {
			author, _ := bc.Engine().Author(block.Header())
		...
		if (common.Rewound == uint64(0)) || ((*head).Number().Uint64() < common.Rewound) {
			if _, err := state.New((*head).Root(), bc.stateCache); err == nil {
				...
				if ok {
					tradingService := engine.GetXPSXService()
					lendingService := engine.GetLendingService()
					if bc.Config().IsTIPXPSX((*head).Number()) && bc.chainConfig.XPoS != nil && (*head).NumberU64() > bc.chainConfig.XPoS.Epoch && tradingService != nil && lendingService != nil {
...
func (bc *BlockChain) HasFullState(block *types.Block) bool {
	...
	engine, _ := bc.Engine().(*XPoS.XPoS)
	if bc.Config().IsTIPXPSX(block.Number()) && bc.chainConfig.XPoS != nil && engine != nil && block.NumberU64() > bc.chainConfig.XPoS.Epoch {
		tradingService := engine.GetXPSXService()
		lendingService := engine.GetLendingService()
		author, _ := bc.Engine().Author(block.Header())
...
func (bc *BlockChain) SaveData() {
	...
	if !bc.cacheConfig.Disabled {
		...
		if bc.Config().IsTIPXPSX(bc.CurrentBlock().Number()) && bc.chainConfig.XPoS != nil && bc.CurrentBlock().NumberU64() > bc.chainConfig.XPoS.Epoch && engine != nil {
			tradingService = engine.GetXPSXService()
		...
		for _, offset := range []uint64{0, 1, triesInMemory - 1} {
			if number := bc.CurrentBlock().NumberU64(); number > offset {
				...
				if bc.Config().IsTIPXPSX(recent.Number()) && bc.chainConfig.XPoS != nil && recent.NumberU64() > bc.chainConfig.XPoS.Epoch && engine != nil {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB, tradingState *tradingstate.TradingStateDB, lendingState *lendingstate.LendingStateDB) (status WriteStatus, err error) {
	...
	var lendingTrieDb *trie.Database
	var lendingService utils.LendingService
	if bc.Config().IsTIPXPSX(block.Number()) && bc.chainConfig.XPoS != nil && block.NumberU64() > bc.chainConfig.XPoS.Epoch && engine != nil {
		tradingService = engine.GetXPSXService()
		if tradingService != nil {
			tradingTrieDb = tradingService.GetStateCache().TrieDB()
		}
	...
	if status == CanonStatTy {
		...
		if bc.chainConfig.XPoS != nil && ((block.NumberU64() % bc.chainConfig.XPoS.Epoch) == (bc.chainConfig.XPoS.Epoch - bc.chainConfig.XPoS.Gap)) {
			err := bc.UpdateM1()
			if err != nil {
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		if bc.Config().IsTIPXPSX(block.Number()) && bc.chainConfig.XPoS != nil && engine != nil && block.NumberU64() > bc.chainConfig.XPoS.Epoch {
			...
			if tradingService != nil && lendingService != nil {
				...
				if (block.NumberU64() % bc.chainConfig.XPoS.Epoch) == 0 {
					if err := tradingService.UpdateMediumPriceBeforeEpoch(block.NumberU64()/bc.chainConfig.XPoS.Epoch, tradingState, statedb); err != nil {
						return i, events, coalescedLogs, err
					}
				} else {
					...
					if block.Number().Uint64()%bc.chainConfig.XPoS.Epoch == common.LiquidateLendingTradeBlock {
				...
				if tradingState != nil && tradingService != nil {
					...
					if gotRoot != expectRoot {
						err = fmt.Errorf("invalid XPSx trading state merke trie got : %s , expect : %s ,parent : %s", gotRoot.Hex(), expectRoot.Hex(), parentRoot.Hex())
						bc.reportBlock(block, nil, err)
						return i, events, coalescedLogs, err
					}
					log.Debug("XPSX Trading State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
				if lendingState != nil && tradingState != nil {
					...
					log.Debug("XPSX Lending State Root", "number", block.NumberU64(), "parent", parentRoot.Hex(), "nextRoot", expectRoot.Hex())
				}
			}
		}
		...
		case CanonStatTy:
			...
			if bc.chainConfig.IsTIPXPSX(block.Number()) && bc.chainConfig.XPoS != nil && block.NumberU64() > bc.chainConfig.XPoS.Epoch {
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	for i := len(newChain) - 1; i >= 0; i-- {
		...
		// prepare set of masternodes for the next epoch
		if bc.chainConfig.XPoS != nil && ((newChain[i].NumberU64() % bc.chainConfig.XPoS.Epoch) == (bc.chainConfig.XPoS.Epoch - bc.chainConfig.XPoS.Gap)) {
			err := bc.UpdateM1()
			if err != nil {
				log.Crit("Error when update masternodes set. Stopping node", "err", err)
			}
	...
	if bc.chainConfig.IsTIPXPSX(commonBlock.Number()) && bc.chainConfig.XPoS != nil && commonBlock.NumberU64() > bc.chainConfig.XPoS.Epoch {
...
func (bc *BlockChain) logExchangeData(block *types.Block) {
	...
	XPSXService := engine.GetXPSXService()
	if XPSXService == nil || !XPSXService.IsSDKNode() {
		return
	}
	txMatchBatchData, err := ExtractTradingTransactions(block.Transactions())
	if err != nil {
	...
	for _, txMatchBatch := range txMatchBatchData {
		...
		for _, txMatch := range txMatchBatch.Data {
			...
			if err := XPSXService.SyncDataToSDKNode(takerOrderInTx, txMatchBatch.TxHash, txMatchTime, currentState, trades, rejectedOrders, &dirtyOrderCount); err != nil {
...
func (bc *BlockChain) reorgTxMatches(deletedTxs types.Transactions, newChain types.Blocks) {
	...
	for _, deletedTx := range deletedTxs {
		if deletedTx.IsTradingTransaction() {
			...
			if err := XPSXService.RollbackReorgTxMatch(deletedTx.Hash()); err != nil {
