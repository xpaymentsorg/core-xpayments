...
func ReadSkeletonSyncStatus(db xpsdb.KeyValueReader) []byte {
	data, _ := db.Get(skeletonSyncStatusKey)
	return data
}

// WriteSkeletonSyncStatus stores the serialized sync status to save at shutdown.
func WriteSkeletonSyncStatus(db xpsdb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSkeletonSyncStatus(db xpsdb.KeyValueWriter) {
...
func ReadSkeletonHeader(db xpsdb.KeyValueReader, number uint64) *types.Header {
...
func WriteSkeletonHeader(db xpsdb.KeyValueWriter, header *types.Header) {
...
func DeleteSkeletonHeader(db xpsdb.KeyValueWriter, number uint64) {
