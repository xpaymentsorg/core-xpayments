...
// Package xpsstats implements the network stats reporting service.
package xpsstats
...
import (
	...
	xpsproto "github.com/xpaymentsorg/go-xpayments/xps/protocols/xps"
)
...
// backend encompasses the bare-minimum functionality needed for xpsstats reporting
type backend interface {
	...
	SyncProgress() xpayments.SyncProgress
}

// fullNodeBackend encompasses the functionality necessary for a full node
// reporting to xpsstats
...
// Service implements an xPayments netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(xpsstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["xps"]; info != nil {
		network = fmt.Sprintf("%d", info.(*xpsproto.NodeInfo).Network)
	} else {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the xpsstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to xpsstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to xpsstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to xpsstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to xpsstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to xpsstats")
