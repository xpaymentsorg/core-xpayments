...
// information related to the xPayments protocol such als blocks, transactions and logs.
...
// `xps_getFilterChanges` polling method that is also used for log filters.
//
// https://xps.wiki/json-rpc/API#xps_newpendingtransactionfilter
...
// It is part of the filter package since polling goes with xps_getFilterChanges.
//
// https://xps.wiki/json-rpc/API#xps_newblockfilter
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(xpayments.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as xpayments.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria xpayments.FilterQuery
...
//
// https://xps.wiki/json-rpc/API#xps_newfilter
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	logs := make(chan []*types.Log)
	logsSub, err := api.events.SubscribeLogs(xpayments.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
...
//
// https://xps.wiki/json-rpc/API#xps_getlogs
...
//
// https://xps.wiki/json-rpc/API#xps_uninstallfilter
...
//
// https://xps.wiki/json-rpc/API#xps_getfilterlogs
...
//
// https://xps.wiki/json-rpc/API#xps_getfilterchanges
