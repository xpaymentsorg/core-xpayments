...
var (
	...
	// errSnapWithoutXps is returned if a peer attempts to connect only on the
	// snap protocol without advertizing the xps main protocol.
	errSnapWithoutXps = errors.New("peer connected on snap without compatible xps support")
)

// peerSet represents the collection of active peers currently participating in
// the `xps` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*xpsPeer // Peers connected on the `xps` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `xps` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `xps`
...
func newPeerSet() *peerSet {
	return &peerSet{
		peers:    make(map[string]*xpsPeer),
		snapWait: make(map[string]chan *snap.Peer),
		snapPend: make(map[string]*snap.Peer),
	}
}

// registerSnapExtension unblocks an already connected `xps` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `xps` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `xps` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `xps`
	if !peer.RunningCap(xps.ProtocolName, xps.ProtocolVersions) {
		return errSnapWithoutXps
	}
	...
	// Inject the peer into an `xps` counterpart is available, otherwise save for later
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *xps.Peer) (*snap.Peer, error) {
...
// registerPeer injects a new `xps` peer into the working set, or returns an error
// if the peer is already known.
func (ps *peerSet) registerPeer(peer *xps.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peer(id string) *xpsPeer {
	ps.lock.RLock()
...
func (ps *peerSet) peersWithoutBlock(hash common.Hash) []*xpsPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	list := make([]*xpsPeer, 0, len(ps.peers))
...
func (ps *peerSet) peersWithoutTransaction(hash common.Hash) []*xpsPeer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
...
// len returns if the current number of `xps` peers in the set. Since the `snap`
// peers are tied to the existence of an `xps` connection, that will always be a
// subset of `xps`.
...
func (ps *peerSet) peerWithHighestTD() *xps.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()
