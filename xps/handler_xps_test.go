...
// testXpsHandler is a mock event handler to listen for inbound network requests
// on the `xps` protocol and convert them into a more easily testable form.
type testXpsHandler struct {
	blockBroadcasts event.Feed
	txAnnounces     event.Feed
	txBroadcasts    event.Feed
}

func (h *testXpsHandler) Chain() *core.BlockChain              { panic("no backing chain") }
func (h *testXpsHandler) TxPool() xps.TxPool                   { panic("no backing tx pool") }
func (h *testXpsHandler) AcceptTxs() bool                      { return true }
func (h *testXpsHandler) RunPeer(*xps.Peer, xps.Handler) error { panic("not used in tests") }
func (h *testXpsHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testXpsHandler) Handle(peer *xps.Peer, packet xps.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected xps packet type in tests: %T", packet))
	}
}

// Tests that peers are correctly accepted (or rejected) based on the advertised
// fork IDs in the protocol handshake.
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, xps.XPS66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	var (
		...
		xpsNoFork, _ = newHandler(&handlerConfig{
			Database:   dbNoFork,
			Chain:      chainNoFork,
		...
		xpsProFork, _ = newHandler(&handlerConfig{
			Database:   dbProFork,
			Chain:      chainProFork,
			TxPool:     newTestTxPool(),
			Merger:     consensus.NewMerger(rawdb.NewMemoryDatabase()),
	...
	xpsNoFork.Start(1000)
	xpsProFork.Start(1000)

	// Clean up everything after ourselves
	defer chainNoFork.Stop()
	defer chainProFork.Stop()

	defer xpsNoFork.Stop()
	defer xpsProFork.Stop()
	...
	peerNoFork := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- xpsNoFork.runXpsPeer(peerProFork, func(peer *xps.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- xpsProFork.runXpsPeer(peerNoFork, func(peer *xps.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = xps.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = xps.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, xps.XPS66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runXpsPeer(sink, func(peer *xps.Peer) error {
		return xps.Handle((*xpsHandler)(handler.handler), peer)
	})
...
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, xps.XPS66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runXpsPeer(src, func(peer *xps.Peer) error {
		return xps.Handle((*xpsHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	for len(seen) < len(insert) {
		...
		case 66:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post xps/66")
			}
...
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, xps.XPS66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i + 1)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runXpsPeer(sourcePeer, func(peer *xps.Peer) error {
			return xps.Handle((*xpsHandler)(source.handler), peer)
		})
		go sink.handler.runXpsPeer(sinkPeer, func(peer *xps.Peer) error {
			return xps.Handle((*xpsHandler)(sink.handler), peer)
		})
	}
...
// Tests that post xps protocol handshake, clients perform a mutual checkpoint
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := xps.NewPeer(xps.XPS66, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := xps.NewPeer(xps.XPS66, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runXpsPeer(local, func(peer *xps.Peer) error {
			return xps.Handle((*xpsHandler)(handler.handler), peer)
		})
	}()
	...
	if checkpoint {
		...
		request := new(xps.GetBlockHeadersPacket66)
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	sinks := make([]*testXpsHandler, peers)
	for i := 0; i < len(sinks); i++ {
		sinks[i] = new(testXpsHandler)
	}
	// Interconnect all the sink handlers with the source handler
	var (
	...
	for i, sink := range sinks {
		...
		sourcePeer := xps.NewPeer(xps.XPS66, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := xps.NewPeer(xps.XPS66, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
...
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, xps.XPS66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := xps.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runXpsPeer(src, func(peer *xps.Peer) error {
		return xps.Handle((*xpsHandler)(source.handler), peer)
	})
