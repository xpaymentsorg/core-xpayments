...
func TestXps2AssembleBlock(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, xpsservice := startXpsService(t, genesis, blocks)
	defer n.Close()

	api := NewConsensusAPI(xpsservice)
	signer := types.NewEIP155Signer(xpsservice.BlockChain().Config().ChainID)
...
func TestXps2AssembleBlockWithAnotherBlocksTxs(t *testing.T) {
	genesis, blocks := generatePreMergeChain(10)
	n, xpsservice := startXpsService(t, genesis, blocks[:9])
	defer n.Close()
...
func TestXps2PrepareAndGetPayload(t *testing.T) {
	// TODO (MariusVanDerWijden) TestXps2PrepareAndGetPayload is currently broken, fixed in upcoming merge-kiln-v2 pr
	/*
		genesis, blocks := generatePreMergeChain(10)
...
func TestXps2NewBlock(t *testing.T) {
	genesis, preMergeBlocks := generatePreMergeChain(10)
	n, xpsservice := startXpsService(t, genesis, preMergeBlocks)
	xpsservice.Merger().ReachTTD()
	defer n.Close()
	...
	xpsservice.BlockChain().SubscribeLogsEvent(newLogCh)
	xpsservice.BlockChain().SubscribeRemovedLogsEvent(rmLogsCh)

	for i := 0; i < 10; i++ {
		statedb, _ := xpsservice.BlockChain().StateAt(parent.Root())
		nonce := statedb.GetNonce(testAddr)
		tx, _ := types.SignTx(types.NewContractCreation(nonce, new(big.Int), 1000000, big.NewInt(2*params.InitialBaseFee), logCode), types.LatestSigner(xpsservice.BlockChain().Config()), testKey)
		xpsservice.TxPool().AddLocal(tx)
		...
		if xpsservice.BlockChain().CurrentBlock().NumberU64() != block.NumberU64()-1 {
...
func startXpsService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *xps.xPayments) {
	t.Helper()
	...
	xpscfg := &xpsconfig.Config{Genesis: genesis, Xpsash: xpsash.Config{PowMode: xpsash.ModeFake}, TrieTimeout: time.Minute, TrieDirtyCache: 256, TrieCleanCache: 256}
	xpsservice, err := xps.New(n, xpscfg)
	if err != nil {
	...
	if _, err := xpsservice.BlockChain().InsertChain(blocks); err != nil {
		n.Close()
		t.Fatal("can't import test blocks:", err)
	}
	xpsservice.SetXpsbase(testAddr)
	xpsservice.SetSynced()
...
func TestFullAPI(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		...
		if xpsservice.BlockChain().CurrentBlock().NumberU64() != payload.Number {
			t.Fatalf("Chain head should be updated")
		}
		parent = xpsservice.BlockChain().CurrentBlock()
	}
