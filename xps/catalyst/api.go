...
func (api *ConsensusAPI) ForkchoiceUpdatedV1(update beacon.ForkchoiceStateV1, payloadAttributes *beacon.PayloadAttributesV1) (beacon.ForkChoiceResponse, error) {
	...
	// reason.
	block := api.xps.BlockChain().GetBlockByHash(update.HeadBlockHash)
	if block == nil {
		// If the head hash is unknown (was not given to us in a newPayload request),
		// we cannot resolve the header, so not much to do. This could be extended in
		// the future to resolve from the `xps` network, but it's an unexpected case
		...
		if merger := api.xps.Merger(); !merger.TDDReached() {
			merger.ReachTTD()
			api.xps.Downloader().Cancel()
		}
		log.Info("Forkchoice requested sync to new head", "number", header.Number, "hash", header.Hash())
		if err := api.xps.Downloader().BeaconSync(api.xps.SyncMode(), header); err != nil {
	...
	if block.Difficulty().BitLen() > 0 || block.NumberU64() == 0 {
		var (
			td  = api.xps.BlockChain().GetTd(update.HeadBlockHash, block.NumberU64())
			ptd = api.xps.BlockChain().GetTd(block.ParentHash(), block.NumberU64()-1)
	...
	if rawdb.ReadCanonicalHash(api.xps.ChainDb(), block.NumberU64()) != update.HeadBlockHash {
		// Block is not canonical, set head.
		if err := api.xps.BlockChain().SetChainHead(block); err != nil {
			return beacon.STATUS_INVALID, err
		}
	} else {
		// If the head block is already in our canonical chain, the beacon client is
		// probably resyncing. Ignore the update.
		log.Info("Ignoring beacon update to old head", "number", block.NumberU64(), "hash", update.HeadBlockHash, "age", common.PrettyAge(time.Unix(int64(block.Time()), 0)), "have", api.xps.BlockChain().CurrentBlock().NumberU64())
	}
	api.xps.SetSynced()
	...
	if update.FinalizedBlockHash != (common.Hash{}) {
		...
		} else if rawdb.ReadCanonicalHash(api.xps.ChainDb(), finalBlock.NumberU64()) != update.FinalizedBlockHash {
	...
	if update.SafeBlockHash != (common.Hash{}) {
		safeBlock := api.xps.BlockChain().GetBlockByHash(update.SafeBlockHash)
		if safeBlock == nil {
			log.Warn("Safe block not available in database")
			return beacon.STATUS_INVALID, errors.New("safe head not available")
		}
		if rawdb.ReadCanonicalHash(api.xps.ChainDb(), safeBlock.NumberU64()) != update.SafeBlockHash {
...
func (api *ConsensusAPI) validForkChoiceResponse(id *beacon.PayloadID) beacon.ForkChoiceResponse {
	currentHash := api.xps.BlockChain().CurrentBlock().Hash()
...
func (api *ConsensusAPI) ExchangeTransitionConfigurationV1(config beacon.TransitionConfigurationV1) (*beacon.TransitionConfigurationV1, error) {
	...
	if ttd.Cmp(config.TerminalTotalDifficulty.ToInt()) != 0 {
		log.Warn("Invalid TTD configured", "gpay", ttd, "beacon", config.TerminalTotalDifficulty)
		return nil, fmt.Errorf("invalid ttd: execution %v consensus %v", ttd, config.TerminalTotalDifficulty)
	}

	if config.TerminalBlockHash != (common.Hash{}) {
		if hash := api.xps.BlockChain().GetCanonicalHash(uint64(config.TerminalBlockNumber)); hash == config.TerminalBlockHash {
...
// NewPayloadV1 creates an Xps1 block, inserts it in the chain, and returns the status of the chain.
func (api *ConsensusAPI) NewPayloadV1(params beacon.ExecutableDataV1) (beacon.PayloadStatusV1, error) {
	...
	if block := api.xps.BlockChain().GetBlockByHash(params.BlockHash); block != nil {
	...
	parent := api.xps.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		...
		if err := api.xps.Downloader().BeaconExtend(api.xps.SyncMode(), block.Header()); err == nil {
	...
	var (
		td  = api.xps.BlockChain().GetTd(parent.Hash(), parent.NumberU64())
		ttd = api.xps.BlockChain().Config().TerminalTotalDifficulty
	)
	if td.Cmp(ttd) < 0 {
	...
	if !api.xps.BlockChain().HasBlockAndState(block.ParentHash(), block.NumberU64()-1) {
		api.remoteBlocks.put(block.Hash(), block.Header())
...
func (api *ConsensusAPI) invalid(err error) beacon.PayloadStatusV1 {
	currentHash := api.xps.BlockChain().CurrentHeader().Hash()
	errorMsg := err.Error()
