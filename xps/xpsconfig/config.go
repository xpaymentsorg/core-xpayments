...
// Package xpsconfig contains the configuration of the XPS and LXS protocols.
package xpsconfig
...
// Config contains configuration options for of the xps and LXS protocols.
type Config struct {
	...
	// canonical chain of all remote peers. Setting the option makes gpay verify the
	// presence of these blocks for every new peer connection.
	PeerRequiredBlocks map[uint64]common.Hash `toml:"-"`

	// Light client options
	LightServ          int  `toml:",omitempty"` // Maximum percentage of time allowed for serving LXS requests
	LightIngress       int  `toml:",omitempty"` // Incoming bandwidth limit for light servers
	LightEgress        int  `toml:",omitempty"` // Outgoing bandwidth limit for light servers
	LightPeers         int  `toml:",omitempty"` // Maximum number of LXS client peers
	...
	// RPCXVMTimeout is the global timeout for xps-call.
	RPCXVMTimeout time.Duration

	// RPCTxFeeCap is the global transaction fee(price * gaslimit) cap for
	// send-transction variants. The unit is xps.
	RPCTxFeeCap float64
