...
// Config are the configuration options for structured logger the XVM
type Config struct {
...
// StructLog is emitted to the XVM each cycle and lists information about the current internal state
...
// StructLogger is an XVM state logger and implements XVMLogger.
//
...
func (l *StructLogger) CaptureStart(env *vm.XVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	l.env = env
}
...
func (l *StructLogger) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	...
	// create a new snapshot of the XVM.
...
func (t *mdLogger) CaptureStart(env *vm.XVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
	if !create {
