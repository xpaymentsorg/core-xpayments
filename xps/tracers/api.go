...
// NewAPI creates a new API definition for the tracing methods of the xPayments service.
...
// chainContext construts the context reader which is used by the xvm for reading
...
type TraceCallConfig struct {
	*logger.Config
	...
	Timeout        *string
	Reexec         *uint64
	StateOverrides *xpsapi.StateOverride
}
...
// TraceChain returns the structured logs created during the execution of XVM
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				...
				blockCtx := core.NewXVMBlockContext(task.block.Header(), api.chainContext(localctx), nil)
...
// XVM and returns them as a JSON object.
...
// TraceBlock returns the structured logs created during the execution of XVM
...
// XVM against a block pulled from the pool of bad ones and returns them as a JSON
// object.
...
// execution of XVM to the local file system and returns a list of files
// to the caller.
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	var (
		...
		vmctx              = core.NewXVMBlockContext(block.Header(), api.chainContext(ctx), nil)
	...
	for i, tx := range block.Transactions() {
		var (
			...
			vmenv     = vm.NewXVM(vmctx, txContext, statedb, chainConfig, vm.Config{})
		)
...
// execution of XVM against a block pulled from the pool of bad ones to the
...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for th := 0; th < threads; th++ {
		pend.Add(1)
		go func() {
			blockCtx := core.NewXVMBlockContext(block.Header(), api.chainContext(ctx), nil)
			defer pend.Done()
	...
	for i, tx := range txs {
		...
		msg, _ := tx.AsMessage(signer, block.BaseFee())
		statedb.Prepare(tx.Hash(), i)
		vmenv := vm.NewXVM(blockCtx, core.NewXVMTxContext(msg), statedb, api.backend.ChainConfig(), vm.Config{})
		if _, err := core.ApplyMessage(vmenv, msg, new(core.GasPool).AddGas(msg.Gas())); err != nil {
			failed = err
			break
		}
...
func (api *API) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		vmenv := vm.NewXVM(vmctx, txContext, statedb, chainConfig, vmConf)
		statedb.Prepare(tx.Hash(), i)
...
// TraceTransaction returns the structured logs created during the execution of XVM
// and returns them as a JSON object.
...
// TraceCall lets you trace a given xps_call. It collects the structured logs
// created during the execution of XVM if the given transaction was added on
// top of the provided block and returns them as a JSON object.
// You can provide -2 as a block number to trace on top of the pending block.
func (api *API) TraceCall(ctx context.Context, args xpsapi.TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceCallConfig) (interface{}, error) {
...
func (api *API) traceTx(ctx context.Context, message core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	var (
		tracer    vm.XVMLogger
		err       error
		txContext = core.NewXVMTxContext(message)
	)
	switch {
	case config == nil:
	...
	vmenv := vm.NewXVM(vmctx, txContext, statedb, api.backend.ChainConfig(), vm.Config{Debug: true, Tracer: tracer, NoBaseFee: true})
	...
	case *logger.StructLogger:
		...
		return &xpsapi.ExecutionResult{
			Gas:         result.UsedGas,
			Failed:      result.Failed(),
			ReturnValue: returnVal,
			StructLogs:  xpsapi.FormatLogs(tracer.StructLogs()),
		}, nil
