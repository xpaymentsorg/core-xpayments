...
// a Gpay console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = xps.getBlock(xps.getTransaction(tx).blockHash);
  var genesis = xps.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.xps.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  xps.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
...
func testCallTracer(tracerName string, dirPath string, t *testing.T) {
	...
	for _, file := range files {
		...
		t.Run(camel(strings.TrimSuffix(file.Name(), ".json")), func(t *testing.T) {
			...
			xvm := vm.NewXVM(context, txContext, statedb, test.Genesis.Config, vm.Config{Debug: true, Tracer: tracer})
			msg, err := tx.AsMessage(signer, nil)
			if err != nil {
				t.Fatalf("failed to prepare transaction for tracing: %v", err)
			}
			st := core.NewStateTransition(xvm, msg, new(core.GasPool).AddGas(tx.Gas()))
...
func TestZeroValueToNotExitCall(t *testing.T) {
	...
	// Create the tracer, the XVM environment and run it
	tracer, err := tracers.New("callTracer", nil)
	if err != nil {
		t.Fatalf("failed to create call tracer: %v", err)
	}
	xvm := vm.NewXVM(context, txContext, statedb, params.MainnetChainConfig, vm.Config{Debug: true, Tracer: tracer})
	msg, err := tx.AsMessage(signer, nil)
	if err != nil {
		t.Fatalf("failed to prepare transaction for tracing: %v", err)
	}
