...
func (b *testBackend) StateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	for idx, tx := range block.Transactions() {
		...
		context := core.NewXVMBlockContext(block.Header(), b.chain, nil)
		if idx == txIndex {
			return msg, context, statedb, nil
		}
		vmenv := vm.NewXVM(context, txContext, statedb, b.chainConfig, vm.Config{})
...
func TestTraceCall(t *testing.T) {
	...
	genesis := &core.Genesis{Alloc: core.GenesisAlloc{
		accounts[0].addr: {Balance: big.NewInt(params.Xps)},
		accounts[1].addr: {Balance: big.NewInt(params.Xps)},
		accounts[2].addr: {Balance: big.NewInt(params.Xps)},
	}}
	genBlocks := 10
	...
	var testSuite = []struct {
		...
		// Standard JSON trace upon the genesis, plain transfer.
		{
			...
			config:    nil,
			expectErr: nil,
			expect: &xpsapi.ExecutionResult{
				Gas:         params.TxGas,
				Failed:      false,
				ReturnValue: "",
				StructLogs:  []xpsapi.StructLogRes{},
			},
		},
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(result, &xpsapi.ExecutionResult{
		Gas:         params.TxGas,
		Failed:      false,
		ReturnValue: "",
		StructLogs:  []xpsapi.StructLogRes{},
	...
	var testSuite = []struct {
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &xpsapi.StateOverride{
					randomAccounts[0].addr: xpsapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Xps)))},
				},
			},
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				//Tracer: &tracer,
				StateOverrides: &xpsapi.StateOverride{
					randomAccounts[2].addr: xpsapi.OverrideAccount{
