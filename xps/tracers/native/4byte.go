...
// 4 byte-identifiers of a tx, and implements vm.XVMLogger.
...
// isPrecompiled returns whether the addr is a precompile. Logic borrowed from newJsTracer in xps/tracers/js/tracer.go
...
// CaptureStart implements the XVMLogger interface to initialize the tracing operation.
func (t *fourByteTracer) CaptureStart(env *vm.XVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
	t.env = env
...
// CaptureState implements the XVMLogger interface to trace a single step of VM execution.
...
// CaptureEnter is called when XVM enters a new scope (via call, create or selfdestruct).
...
// CaptureExit is called when XVM exits a scope, even if the scope didn't
// execute any code.
func (t *fourByteTracer) CaptureExit(output []byte, gasUsed uint64, err error) {
}

// CaptureFault implements the XVMLogger interface to trace an execution fault.
