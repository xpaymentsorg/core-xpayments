...
// the process. This is usually the case when Gpay is shutting down and some events
...
// data on disk when two subchains are joined togpayer.
...
//
// Opposed to the original xPayments block synchronization which is trustless (and
...
// not from within (vs. legacy xPayments sync).
//
...
type skeleton struct {
	db     xpsdb.Database // Database backing the skeleton
...
func newSkeleton(db xpsdb.Database, peers *peerSet, drop peerDropFn, filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Gpay is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Gpay requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
	...
	if status := rawdb.ReadSkeletonSyncStatus(s.db); len(status) > 0 {
		...
		if err := json.Unmarshal(status, s.progress); err != nil {
			...
			for len(s.progress.Subchains) > 0 {
				// If the last chain is above the new head, delete altogpayer
...
func (s *skeleton) saveSyncStatus(db xpsdb.KeyValueWriter) {
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, xps.BlockHeadersMsg, targetTTL))
	}
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	case <-timeoutTimer.C:
		...
		s.peers.rates.Update(peer.id, xps.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*xps.BlockHeadersPacket)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, xps.BlockHeadersMsg, res.Time, len(headers))
