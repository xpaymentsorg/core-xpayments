...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *xps.Response) (*xps.Request, error) {
	...
	if headers == nil {
		...
		if len(p.headers) > int(origin) { // Don't serve headers if we're missing the origin
			for i := 0; i < amount; i++ {
				...
				// attack by sending/receiving those over xps.
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool, chan *xps.Response) (*xps.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash, chan *xps.Response) (*xps.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash, chan *xps.Response) (*xps.Request, error) {
...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, xps.XPS66, peer, log.New("id", peer.id)))
		}
		...
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id, xps.XPS66, tt.newPeer, log.New("id", tt.newPeer.id))); err != nil {
