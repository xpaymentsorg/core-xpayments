...
var (
	...
	// emptyCode is the known hash of the empty XVM bytecode.
...
type accountTask struct {
	...
	genBatch xpsdb.Batch     // Batch used by the node generator
...
// Syncer is an xPayments account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db xpsdb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        xpsdb.Batch        // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the xPayments state over the
// snap protocol.
func NewSyncer(db xpsdb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = xpsdb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for _, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = xpsdb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > xpsdb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > xpsdb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > xpsdb.IdealBatchSize {
