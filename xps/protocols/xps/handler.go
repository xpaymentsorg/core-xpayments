...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `xps` protocol. The handler
...
// MakeProtocols constructs the P2P protocol definitions for `xps`.
...
// NodeInfo represents a short summary of the `xps` sub-protocol metadata
...
// nodeInfo retrieves some `xps` protocol metadata about the running host node.
...
// Handle is invoked whenever an `xps` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `xps`", "err", err)
			return err
		}
	}
}
...
var xps66 = map[uint64]msgHandler{
...
func handleMessage(backend Backend, peer *Peer) error {
	...
	var handlers = xps66
	//if peer.Version() >= XPS67 { // Left in as a sample when new protocol is added
	//	handlers = xps67
	//}
