...
// enrEntry is the ENR entry which advertises `xps` protocol on the discovery.
...
// StartENRUpdater starts the `xps` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `xps` ENR entry based on the current state of the chain.
