...
// testBackend is a mock implementation of the live xPayments message handler. Its
// purpose is to allow testing the request/reply workflows and wire serialization
// in the `xps` protocol without actually doing any data processing.
...
func newTestBackendWithGenerator(blocks int, generator func(int, *core.BlockGen)) *testBackend {
	...
	chain, _ := core.NewBlockChain(db, nil, params.TestChainConfig, xpsash.NewFaker(), vm.Config{}, nil, nil)

	bs, _ := core.GenerateChain(params.TestChainConfig, chain.Genesis(), xpsash.NewFaker(), db, blocks, generator)
...
func TestGetBlockHeaders66(t *testing.T) { testGetBlockHeaders(t, XPS66) }
...
func TestGetBlockBodies66(t *testing.T) { testGetBlockBodies(t, XPS66) }
...
func TestGetNodeData66(t *testing.T) { testGetNodeData(t, XPS66) }
...
func testGetNodeData(t *testing.T, protocol uint) {
	...
	generator := func(i int, block *core.BlockGen) {
		switch i {
		case 0:
			// In block 1, the test bank sends account #1 some xps.
			...
			block.AddTx(tx)
		case 1:
			// In block 2, the test bank sends some more xps to account #1.
...
func TestGetBlockReceipts66(t *testing.T) { testGetBlockReceipts(t, XPS66) }
