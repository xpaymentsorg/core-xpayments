...
// ProtocolName is the official short name of the `xps` protocol used during
// devp2p capability negotiation.
const ProtocolName = "xps"

// ProtocolVersions are the supported versions of the `xps` protocol (first
// is primary).
var ProtocolVersions = []uint{XPS66}

// protocolLengths are the number of implemented message corresponding to
// different protocol versions.
var protocolLengths = map[uint]uint64{XPS66: 17}
...
// Packet represents a p2p message in the `xps` protocol.
type Packet interface {
	Name() string // Name returns a string corresponding to the message type.
	Kind() byte   // Kind returns the message type.
}

// StatusPacket is the network packet for the status message for xps/64 and later.
...
// GetBlockHeadersPacket66 represents a block header query over xps/66
...
// BlockHeadersPacket represents a block header response over xps/66.
...
// GetBlockBodiesPacket represents a block body query over xps/66.
...
// BlockBodiesPacket is the network packet for block content distribution over xps/66.
...
// BlockBodiesRLPPacket66 is the BlockBodiesRLPPacket over xps/66
...
// GetNodeDataPacket represents a trie node data query over xps/66.
...
// NodeDataPacket is the network packet for trie node data distribution over xps/66.
...
// GetReceiptsPacket represents a block receipts query over xps/66.
...
// ReceiptsPacket is the network packet for block receipts distribution over xps/66.
...
// ReceiptsPacket66 is the xps-66 version of ReceiptsRLPPacket
...
// PooledTransactionsPacket is the network packet for transaction distribution over xps/66.
...
// PooledTransactionsRLPPacket66 is the xps/66 form of PooledTransactionsRLPPacket
