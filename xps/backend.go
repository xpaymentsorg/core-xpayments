...
// xPayments implements the xPayments full node service.
...
func New(stack *node.Node, config *xpsconfig.Config) (*xPayments, error) {
	...
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run xps.xPayments in light sync mode, use lxs.LightxPayments")
	}
...
func (s *xPayments) Xpsbase() (eb common.Address, err error) {
	s.lock.RLock()
	xpsbase := s.xpsbase
	s.lock.RUnlock()
...
func (s *xPayments) isLocalBlock(header *types.Header) bool {
	...
	// Check whether the given address is xpsbase.
	s.lock.RLock()
	xpsbase := s.xpsbase
	s.lock.RUnlock()
	if author == xpsbase {
...
func (s *xPayments) SetXpsbase(xpsbase common.Address) {
	s.lock.Lock()
	s.xpsbase = xpsbase
	s.lock.Unlock()

	s.miner.SetXpsbase(xpsbase)
}
