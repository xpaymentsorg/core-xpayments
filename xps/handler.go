...
import (
	...
	"github.com/xpaymentsorg/go-xpayments/xps/fetcher"
...
// support all the operations needed by the xPayments chain protocols.
...
type handlerConfig struct {
	Database   xpsdb.Database            // Database for direct sync insertions
	Chain      *core.BlockChain          // Blockchain to serve data from
	TxPool     txPool                    // Transaction pool to propagate from
	Merger     *consensus.Merger         // The manager for xps1/2 transition
...
// newHandler returns a handler for all xPayments chain management protocol.
func newHandler(config *handlerConfig) (*handler, error) {
	...
	inserter := func(blocks types.Blocks) (int, error) {
		...
		if h.merger.TDDReached() {
			...
			// pow blocks. Spec: https://github.com/xpayments/EIPs/blob/master/EIPS/eip-3675.md#halt-the-importing-of-pow-blocks
...
// runXpsPeer registers an xps peer into the joint xps/snap peerset, adds it to
// various subsistems and starts handling messages.
func (h *handler) runXpsPeer(peer *xps.Peer, handler xps.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("xPayments handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("xPayments peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("xPayments peer registration failed", "err", err)
		return err
	}
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in xps syncer", "err", err)
		return err
	}
	if snap != nil {
	...
	if h.checkpointHash != (common.Hash{}) {
		...
		go func() {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*xps.BlockHeadersPacket))
				if len(headers) == 0 {
...
// runSnapExtension registers a `snap` peer into the joint xps/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `xps`, all subsystem registrations and lifecycle management will be done by
// the main `xps` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("xPayments peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `xps` peer if it exists
	logger.Debug("Removing xPayments peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("xPayments peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("xPayments protocol stopped")
}
...
func (h *handler) BroadcastTransactions(txs types.Transactions) {
	var (
		...
		txset = make(map[*xpsPeer][]common.Hash) // Set peer->hash to transfer directly
		annos = make(map[*xpsPeer][]common.Hash) // Set peer->hash to announce
