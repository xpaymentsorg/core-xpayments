...
func (XPSx *XPSX) CommitOrder(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	XPSxSnap := tradingStateDB.Snapshot()
	dbSnap := statedb.Snapshot()
	trades, rejects, err := XPSx.ApplyOrder(header, coinbase, chain, statedb, tradingStateDB, orderBook, order)
	if err != nil {
		tradingStateDB.RevertToSnapshot(XPSxSnap)
...
func (XPSx *XPSX) ApplyOrder(header *types.Header, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
	...
	if order.Status == tradingstate.OrderStatusCancelled {
		err, reject := XPSx.ProcessCancelOrder(header, tradingStateDB, statedb, chain, coinbase, orderBook, order)
		if err != nil || reject {
	...
	if orderType == tradingstate.Market {
		...
		trades, rejects, err = XPSx.processMarketOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
		...
		trades, rejects, err = XPSx.processLimitOrder(coinbase, chain, statedb, tradingStateDB, orderBook, order)
		if err != nil {
...
func (XPSx *XPSX) processMarketOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = XPSx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
		...
		log.Debug("processMarketOrder ", "side", side, "bestPrice", bestPrice, "quantityToTrade", quantityToTrade, "volume", volume)
		for quantityToTrade.Cmp(zero) > 0 && bestPrice.Cmp(zero) > 0 {
			quantityToTrade, newTrades, newRejects, err = XPSx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, bestPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (XPSx *XPSX) processLimitOrder(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, orderBook common.Hash, order *tradingstate.OrderItem) ([]map[string]string, []*tradingstate.OrderItem, error) {
	var (
		trades     []map[string]string
		newTrades  []map[string]string
		rejects    []*tradingstate.OrderItem
	...
	if side == tradingstate.Bid {
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(minPrice) >= 0 && minPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = XPSx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Ask, orderBook, minPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
		...
		for quantityToTrade.Cmp(zero) > 0 && price.Cmp(maxPrice) <= 0 && maxPrice.Cmp(zero) > 0 {
			...
			quantityToTrade, newTrades, newRejects, err = XPSx.processOrderList(coinbase, chain, statedb, tradingStateDB, tradingstate.Bid, orderBook, maxPrice, quantityToTrade, order)
			if err != nil {
				return nil, nil, err
			}
			trades = append(trades, newTrades...)
...
func (XPSx *XPSX) processOrderList(coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, tradingStateDB *tradingstate.TradingStateDB, side string, orderBook common.Hash, price *big.Int, quantityStillToTrade *big.Int, order *tradingstate.OrderItem) (*big.Int, []map[string]string, []*tradingstate.OrderItem, error) {
	...
	for quantityToTrade.Sign() > 0 {
		...
		if oldestOrder.QuoteToken.String() != common.XPSNativeAddress {
			quotePrice = tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(oldestOrder.QuoteToken, common.HexToAddress(common.XPSNativeAddress)))
			log.Debug("TryGet quotePrice QuoteToken/XPS", "quotePrice", quotePrice)
			if quotePrice == nil || quotePrice.Sign() == 0 {
				inversePrice := tradingStateDB.GetLastPrice(tradingstate.GetTradingOrderBookHash(common.HexToAddress(common.XPSNativeAddress), oldestOrder.QuoteToken))
				quoteTokenDecimal, err := XPSx.GetTokenDecimal(chain, statedb, oldestOrder.QuoteToken)
				if err != nil || quoteTokenDecimal.Sign() == 0 {
					return nil, nil, nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", oldestOrder.QuoteToken.String(), err)
				}
				log.Debug("TryGet inversePrice XPS/QuoteToken", "inversePrice", inversePrice)
				if inversePrice != nil && inversePrice.Sign() > 0 {
					quotePrice = new(big.Int).Mul(common.BasePrice, quoteTokenDecimal)
					quotePrice = new(big.Int).Div(quotePrice, inversePrice)
					log.Debug("TryGet quotePrice after get inversePrice XPS/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteTokenDecimal)
				}
			}
		} else {
			quotePrice = common.BasePrice
		}
		tradedQuantity, rejectMaker, settleBalanceResult, err := XPSx.getTradeQuantity(quotePrice, coinbase, chain, statedb, order, &oldestOrder, maxTradedQuantity)
...
func (XPSx *XPSX) getTradeQuantity(quotePrice *big.Int, coinbase common.Address, chain consensus.ChainContext, statedb *state.StateDB, takerOrder *tradingstate.OrderItem, makerOrder *tradingstate.OrderItem, quantityToTrade *big.Int) (*big.Int, bool, *tradingstate.SettleBalance, error) {
	baseTokenDecimal, err := XPSx.GetTokenDecimal(chain, statedb, makerOrder.BaseToken)
	...
	quoteTokenDecimal, err := XPSx.GetTokenDecimal(chain, statedb, makerOrder.QuoteToken)
	if err != nil || quoteTokenDecimal.Sign() == 0 {
		return tradingstate.Zero, false, nil, fmt.Errorf("Fail to get tokenDecimal. Token: %v . Err: %v", makerOrder.QuoteToken.String(), err)
	}
	if makerOrder.QuoteToken.String() == common.XPSNativeAddress {
...
func GetTradeQuantity(takerSide string, takerFeeRate *big.Int, takerBalance *big.Int, makerPrice *big.Int, makerFeeRate *big.Int, makerBalance *big.Int, baseTokenDecimal *big.Int, quantityToTrade *big.Int) (*big.Int, bool) {
	if takerSide == tradingstate.Bid {
		...
		takerFee = big.NewInt(0).Div(takerFee, common.XPSXBaseFee)
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.XPSXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.XPSXBaseFee, takerFeeRate))
		...
		if takerBalance.Cmp(takerOutTotal) >= 0 && makerBalance.Cmp(makerOutTotal) >= 0 {
			...
			newQuantityTrade = new(big.Int).Mul(newQuantityTrade, common.XPSXBaseFee)
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, new(big.Int).Add(common.XPSXBaseFee, makerFeeRate))
			newQuantityTrade = new(big.Int).Div(newQuantityTrade, makerPrice)
			log.Debug("Reject order maker , not enough balance ", "makerBalance", makerBalance, " makerOutTotal", makerOutTotal)
...
func DoSettleBalance(coinbase common.Address, takerOrder, makerOrder *tradingstate.OrderItem, settleBalance *tradingstate.SettleBalance, statedb *state.StateDB) error {
	...
	//log.Debug("ApplyXPSXMatchedTransaction settle fee for relayers",
	//	"takerRelayerOwner", takerExOwner,
	//	"takerFeeToken", quoteToken, "takerFee", settleBalanceResult[takerAddr][XPSx.Fee].(*big.Int),
	//	"makerRelayerOwner", makerExOwner,
	//	"makerFeeToken", quoteToken, "makerFee", settleBalanceResult[makerAddr][XPSx.Fee].(*big.Int))
	// takerFee
...
func (XPSx *XPSX) ProcessCancelOrder(header *types.Header, tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB, chain consensus.ChainContext, coinbase common.Address, orderBook common.Hash, order *tradingstate.OrderItem) (error, bool) {
	...
	baseTokenDecimal, err := XPSx.GetTokenDecimal(chain, statedb, order.BaseToken)
	if err != nil || baseTokenDecimal.Sign() == 0 {
		log.Debug("Fail to get tokenDecimal ", "Token", order.BaseToken.String(), "err", err)
		return err, false
	}
	// order: basic order information (includes orderId, orderHash, baseToken, quoteToken) which user send to XPSx to cancel order
	...
	tokenCancelFee, tokenPriceInXPS := common.Big0, common.Big0
	if !chain.Config().IsTIPXPSXCancellationFee(header.Number) {
		tokenCancelFee = getCancelFeeV1(baseTokenDecimal, feeRate, &originOrder)
	} else {
		tokenCancelFee, tokenPriceInXPS = XPSx.getCancelFee(chain, statedb, tradingStateDB, &originOrder, feeRate)
	}
	...
	// relayers pay XPS for masternode
	...
	extraData, _ := json.Marshal(struct {
		CancelFee       string
		TokenPriceInXPS string
	}{
		CancelFee:       tokenCancelFee.Text(10),
		TokenPriceInXPS: tokenPriceInXPS.Text(10),
	})
...
// deprecated after hardfork at TIPXPSXCancellationFee
func getCancelFeeV1(baseTokenDecimal *big.Int, feeRate *big.Int, order *tradingstate.OrderItem) *big.Int {
	...
	if order.Side == tradingstate.Ask {
		...
		cancelFee = new(big.Int).Div(cancelFee, common.XPSXBaseCancelFee)
	} else {
		// BUY 1 BTC => XPS with Price : 10000
...
// return tokenQuantity, tokenPriceInXPS
func (XPSx *XPSX) getCancelFee(chain consensus.ChainContext, statedb *state.StateDB, tradingStateDb *tradingstate.TradingStateDB, order *tradingstate.OrderItem, feeRate *big.Int) (*big.Int, *big.Int) {
	if feeRate == nil || feeRate.Sign() == 0 {
		return common.Big0, common.Big0
	}
	cancelFee := big.NewInt(0)
	tokenPriceInXPS := big.NewInt(0)
	var err error
	if order.Side == tradingstate.Ask {
		cancelFee, tokenPriceInXPS, err = XPSx.ConvertXPSToToken(chain, statedb, tradingStateDb, order.BaseToken, common.RelayerCancelFee)
	} else {
		cancelFee, tokenPriceInXPS, err = XPSx.ConvertXPSToToken(chain, statedb, tradingStateDb, order.QuoteToken, common.RelayerCancelFee)
	}
	if err != nil {
		return common.Big0, common.Big0
	}
	return cancelFee, tokenPriceInXPS
}

func (XPSx *XPSX) UpdateMediumPriceBeforeEpoch(epochNumber uint64, tradingStateDB *tradingstate.TradingStateDB, statedb *state.StateDB) error {
	...
	if XPSx.IsSDKNode() {
		if err := XPSx.LogEpochPrice(epochNumber, epochPriceResult); err != nil {
...
// orderbook hash genereted from baseToken, quoteToken at xpaymentsorg/XPSx/tradingstate/common.go:214
func (XPSx *XPSX) LogEpochPrice(epochNumber uint64, epochPriceResult map[common.Hash]*big.Int) error {
	db := XPSx.GetMongoDB()
	db.InitBulk()
