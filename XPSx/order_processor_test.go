...
func Test_getCancelFee(t *testing.T) {
	...
	XPSx.SetTokenDecimal(testTokenB, tokenBDecimal)
	...
	tradingStateDb.SetMediumPriceBeforeEpoch(tradingstate.GetTradingOrderBookHash(common.HexToAddress(common.XPSNativeAddress), testTokenB), tokenBDecimal)
	...
	tests := []struct {
		...
		// test getCancelFee: SELL
		{
			"TokenA/XPS test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
		...
		// test getCancelFee:: BUY
		{
			"TokenA/XPS test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
		...
		// test getCancelFee: SELL
		{
			"XPS/TokenA test getCancelFee:: SELL",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					BaseToken:  common.HexToAddress(common.XPSNativeAddress),
		...
		// test getCancelFee:: BUY
		{
			"XPS/TokenA test getCancelFee:: BUY",
			CancelFeeArg{
				feeRate: new(big.Int).SetUint64(10), // 10/10000= 0.1%
				order: &tradingstate.OrderItem{
					Quantity:   new(big.Int).SetUint64(10000),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got, _ := XPSx.getCancelFee(nil, nil, tradingStateDb, tt.args.order, tt.args.feeRate); !reflect.DeepEqual(got, tt.want) {
	...
	if fee, _ := XPSx.getCancelFee(nil, nil, tradingStateDb, tokenCOrder.order, tokenCOrder.feeRate); fee != nil && fee.Sign() != 0 {
	...
	XPSx.SetTokenDecimal(testTokenD, big.NewInt(0))
	...
	if fee, _ := XPSx.getCancelFee(nil, nil, tradingStateDb, tokenDOrder.order, tokenDOrder.feeRate); fee != nil && fee.Sign() != 0 {
		t.Errorf("getCancelFee() = %v, want %v", fee, common.Big0)
	}
