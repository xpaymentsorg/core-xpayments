...
func VerifyBalance(statedb *state.StateDB, XPSxStateDb *TradingStateDB, order *types.OrderTransaction, baseDecimal, quoteDecimal *big.Int) error {
	var quotePrice *big.Int
	if order.QuoteToken().String() != common.XPSNativeAddress {
		quotePrice = XPSxStateDb.GetLastPrice(GetTradingOrderBookHash(order.QuoteToken(), common.HexToAddress(common.XPSNativeAddress)))
		log.Debug("TryGet quotePrice QuoteToken/XPS", "quotePrice", quotePrice)
		if quotePrice == nil || quotePrice.Sign() == 0 {
			inversePrice := XPSxStateDb.GetLastPrice(GetTradingOrderBookHash(common.HexToAddress(common.XPSNativeAddress), order.QuoteToken()))
			...
			if inversePrice != nil && inversePrice.Sign() > 0 {
				...
				log.Debug("TryGet quotePrice after get inversePrice XPS/QuoteToken", "quotePrice", quotePrice, "quoteTokenDecimal", quoteDecimal)
			}
		}
	} else {
